...
`archive/<source-prefix>/`は過去系列をmanifestとartifactの組で保持する監査履歴であり、再開、追記、現行staging判定には使用しない。旧`dbedb941`系列は`archive/dbedb941/`、v31最終系列は`archive/f24c09d2/`へ固定している。`archive/dd0cbdb-failed-rpc-order/`は、v32 reinstall直後の検査でCustom RPCの順序digest不一致を検出し、activation前に再reinstallした未使用installの証跡である。2026-07-31に確認したDeposit ID衝突と未救済5 TICRC1は失効した旧証跡として`incident-2026-07-31-deposit-id-collision.json`へ固定する。
