...
初回contract deployだけ一時EOAを使用し、deployerへroleを残さない。以後のBase管理操作はBridge Canisterがrole別derivationで導出するGovernance Operator、Runtime Administrator、Independent Cancellerから送信する。production IC操作は`BRIDGE_ICP_IDENTITY`とICP CLIへ統一し、`dfx`を使用しない。Timelock activationはseal時に固定したproduction controllerがcontrollerである間、同controller prepare、匿名relay、固定confirmation relayer confirmによるschedule/execute二段階とし、各段階でlive preflightを再実行する。固定principalをcontrollerから外した後のactivationは既存Governance principal認可に切り替わる。失敗または曖昧結果ではIC/Base pauseを維持し、同じsigned transactionを追跡する。
