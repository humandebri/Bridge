...
Gate B uses a schema 4 manifest containing exactly the six Gate A files plus `rpc-e2e.json`, `monitor-drill.json`, `initial-operational-parameters.json`, `provider-independence.json`, `ui-assets.json`, `gate-a-receipt.json`, and `post-gate-a-policy-transition.json`. Initial evidence binds exact schedule/execute calldata gas estimates, at least ten distinct Finalized fee blocks, and the live idle cycles burn observation. The validator re-derives the fixed fee/cycles ceilings and requires an exact profile match. The policy transition connects the immutable Gate A receipt source and deployed identities/code hashes to the current reviewed release-policy source. Gate B sets `parent_gate_a_manifest_sha256` to the Gate A hash.
...
稼働後のGate Cでは、`monitoring-receipt.json`と`keeper-drill.json`を7日・各10件以上の本番計測およびoperation continuity証跡と合わせて要求する。これらはunpauseを認可するGate Bには含めない。
...
`controller-handover.json`と`sns-upgrade.json`はGate C合格後に作成する。Gate Bからactivation完了まではGate A receiptのproduction installer identityを単独controllerとして維持し、Root-only controllerを要求しない。Gate C後のhandoverはrequest transcript、final controller set、module hash、active lifecycle、storage integrity、operation continuityを別途検証する。

Activation submission and receipt files are deliberately outside the fixed thirteen-artifact Gate B bundle. A schema v3 submission records the fixed generic-function proposal only; it is not success evidence. A schema v4 activation receipt is created with exclusive-create semantics only after an authenticated executed proposal, exact live function registry target, production installer sole-controller/module binding, authenticated `get_activation_status`, and the Canister's independently confirmed Finalized Base transaction all agree. Execute receipts additionally hash-bind the prior verified schedule receipt.
