...
Gate Bの`profile.json`は後者を基礎に、governance EVM fee 8項目、`cycles_floor`、`settlement_cycle_ceiling`だけをraw計測の導出値へ置換する。Gate B validatorはこの10項目をBootstrap値へ正規化して`post_deploy_profile_sha256`を照合し、さらに`deployment_block`を0へ戻したRFC 8785 canonical hashを`gate_a_profile_sha256`へ照合するため、その他のfield driftはmanifestを作り直しても拒否される。
Gate B uses a new manifest containing those six files plus `rpc-e2e.json`, `gate-a-receipt.json`, `controller-handover.json`, `sns-upgrade.json`, `monitor-drill.json`, `keeper-drill.json`, `monitoring-receipt.json`, `fee-cycles-measurements.json`, `provider-independence.json`, and `ui-assets.json`. `fee-cycles-measurements.json` is the complete schema v3 raw measurement evidence accepted by `bridge-profile derive`, not a self-reported sample summary. Every sample carries its own observation timestamp and unique source reference; Gate B re-derives every final fee and cycles parameter and requires an exact profile match. The receipt binds the Gate A manifest hash, release/source identity, Gate A profile hash, post-deployment profile hash, and both code hashes. Gate B also sets `parent_gate_a_manifest_sha256` to that Gate A hash.
