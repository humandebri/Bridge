...
Create `controller-handover.json` and `sns-upgrade.json` only when operators separately approve handover timing. From Gate B through initial activation completion, retain the Gate A receipt's production installer identity as sole controller; do not require Root-only control. Handover authorization is independent of Gate C and `fee-cycles-measurements.json`, binding instead to Gate B `initial-operational-parameters.json`, seal receipt, hash-linked schedule/execute receipts, and live RuntimeBinding from signature-verified queries. The completion receipt also stores source revision/tree and SHA-256 values for Gate B manifest, seal, schedule, and execute, with typed validation rebinding them to original fixed bytes. The live module must match current profile Wasm through `post-gate-a-policy-transition.json` and normal upgrade receipts, not the initial install hash. Operators manually refresh attestation through the fixed confirmation relayer before running the handover driver; the driver performs no refresh and accepts only fresh authenticated values. Immediately before submission, retrieve controllers again and require only the production installer, Activated state, unpaused Base Deposits/Withdrawals and IC Deposits, sufficient reserve, and storage integrity `ok`. Save pre/post-change management status, module, RuntimeBinding, lifecycle, activation status/attestation, storage integrity, and record/audit counts with raw response digests, verifying nonregression without requiring empty state.

Activation submission and receipt files deliberately remain outside the fixed thirteen-artifact Gate B bundle. Initial activation uses schema 1 controller authorization, preparation, confirmation, and completion receipts binding the sole production installer controller, certified `get_activation_status`, and Canister-independently verified Finalized Base transactions. Do not use schema v3 generic-function submissions or schema v4 SNS proposal activation receipts for initial activation. The current schema v4 verifier retains production-installer controller binding; before use for post-handover reactivation, update it to Root-controller/handover-completion binding and obtain separate review. In both receipt paths, execute receipts hash-bind verified schedule receipts.
