...
`bridge-canister`はstable schema v25・record wire v21の単一SQLite DBへ状態を保存し、owner sequence型Deposit API、状態照会、ICRC Ledger、EVM RPC、threshold ECDSA、運用管理APIを接続する。
...
Mint用ETH reserve、gas見積り、nonce、raw transaction、rebroadcast、replacementは存在しない。Canisterが送るBase governance transactionだけが独立したGovernance Operator、ETH floor、nonce、rebroadcast、fee bumpを使用する。
...
3. `bridge-canister`をdeployし、`Running`と`get_bridge_status`のschema version 25、全count 0を確認する。
