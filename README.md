...
`bridge-canister`はstable schema v36・record wire v29の単一SQLite DBへ状態を保存し、owner sequence型Deposit API、状態照会、ICRC Ledger、EVM RPC、threshold ECDSA、運用管理APIを接続する。
...
本番初回deployまではstable schemaを直接置換し、production向けの旧schema migration、dual-read、fallbackを追加しない。現行v36以外・未知schema・未知wireはfail closedとする。唯一の例外として`test-deployment` staging Wasmだけがreview済みv33／wire v28からv36／wire v29への一方向migrationを持つ。`reinstall`はstate破棄を明示承認した使い捨て環境に限定する。
