...
本番初回deployまではstable schemaを直接置換し、旧schema向けmigration、dual-read、fallbackを追加しない。現行v34以外・未知schema・未知wireはfail closedとし、`reinstall`はstate破棄を明示承認した使い捨て環境に限定する。
