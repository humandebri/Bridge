...
`bridge-canister`はstable schema v32・record wire v28の単一SQLite DBへ状態を保存し、owner sequence型Deposit API、状態照会、ICRC Ledger、EVM RPC、threshold ECDSA、運用管理APIを接続する。
...
3. `bridge-canister`をdeployし、`Running`と`get_bridge_status`のschema version 32、全count 0を確認する。
...
本番初回deployまではstable schemaを直接置換し、旧schema向けmigration、dual-read、fallbackを追加しない。現行v32以外・未知schema・未知wireはfail closedとし、`reinstall`はstate破棄を明示承認した使い捨て環境に限定する。
