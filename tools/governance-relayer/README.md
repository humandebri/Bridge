...
`IC_IDENTITY_PEM`は`confirm`、`run`、`prepare`、`replace`、activation prepare、`refresh-attestation`、緊急操作に必要である。初回activation prepareにはseal時に固定したproduction controller identityを使う。Confirmed executeで内部のbootstrap activation authorityが消費された後は、外部controller設定の変更時期にかかわらずGovernance identityだけを使う。`confirm`、`run`、attestation refreshにはrelease profileへ固定した専用confirmation relayer identityを使う。Service FeeにはGovernance identityを使い、pause、記録済みTimelock cancel、`drain-emergency`にはGovernanceまたはpause identityを使える。`status`とraw transactionの`relay`は匿名のままである。confirmation callerの報告は信用せず、保存済みoperation IDと署名generation hashの一致をRPC前に検査し、receiptや状態値はCanisterが独立観測する。
