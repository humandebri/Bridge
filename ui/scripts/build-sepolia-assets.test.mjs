...
describe("Base Sepolia asset profile template", () => {
  ...
  it("deploys production from a standalone UI receipt and live v36 upgrade evidence", async () => {
    ...
    expect(productionAssets).toContain('"verify-production-current-ui-live"')
    expect(productionAssets).toContain("BRIDGE_UI_CONTROLLER_MODE")
    expect(productionAssets).not.toContain("BRIDGE_CHECKPOINT_EVIDENCE")
    expect(productionAssets).toContain("BRIDGE_RELEASE_BUNDLE")
