...
for (const key of ["bridgeCanisterId", "ledgerCanisterId", "indexCanisterId"]) {
  if (
    ...
    profile[key].startsWith("REPLACE_") ||
    productionIds.has(profile[key])
  ) {
...
for (const key of ["bridgeAddress", "bsnsAddress", "timelockAddress", "expected_bridge_signer"]) {
  if (!/^0x[0-9a-fA-F]{40}$/.test(profile[key] ?? ""))
    throw new Error(`Asset build requires ${key}`)
}
...
if (
  ...
  throw new Error("Asset build requires a nonzero minimumWithdrawalId")
}
