...
test("deposits through the real ledger, canister, and Anvil contract", async ({
  ...
  await expect
    .poll(
      async () => {
        ...
        return {
          bound: state.indexLedgerId === state.ledgerId,
          synced: state.indexBalance === state.ledgerBalance,
        }
      },
      { timeout: 30_000 },
    )
    .toEqual({ bound: true, synced: true })
  ...
  await expect(
    page.getByText("Live availability is unknown until current status checks succeed."),
  ).toBeHidden()
  ...
  await expect(
    page.getByRole("dialog").getByRole("button", { name: "Disconnect Plug", exact: true }),
  ).toBeVisible()
  await page.getByRole("button", { name: "Close confirmation" }).click()
  ...
  await expect
    .poll(async () => BigInt((await controlState(request)).bsnsBalance))
    .toBe(199_000_000n)
  ...
  await expect
    .poll(async () => BigInt((await controlState(request)).bsnsBalance))
    .toBe(298_000_000n)
  const upgrade = (await postControl(request, "/test/upgrade", {})) as {
    before: unknown
    after: unknown
  }
  ...
  expect(BigInt(afterDeposit.indexBlocksSynced)).toBeGreaterThanOrEqual(
  ...
  for (const heading of [
    ...
    await expect(
      page.getByText(heading, { exact: true }).filter({ visible: true }).first(),
    ).toBeVisible()
  }
  ...
  if (!nextStepHeaderBox || !completedDepositNextStepBox)
    throw new Error("History Next step alignment could not be measured")
  ...
  await expect
    .poll(async () => BigInt((await controlState(request)).withdrawalCount))
    .toBe(BigInt(beforeWithdrawal.withdrawalCount) + 1n)
  await expect
    .poll(() => [...bridgeUpdateObserver.acceptedCalls])
    .toEqual([expect.objectContaining({ method: "notify_withdrawal" })])
  await postControl(request, "/test/set-ledger-available", { available: false })
  bridgeUpdateGate.release()
  await expect
    .poll(
      async () => {
        const state = (await postControl(request, "/test/latest-withdrawal-state", {})) as {
          phase: string
  ...
  expect([...new Set(bridgeUpdateObserver.attempts.map((call) => call.method))]).toEqual([
    "notify_withdrawal",
    "continue_withdrawal",
  ])
  ...
  await expect
    .poll(async () => (await controlState(request)).receiptCalls, { timeout: 45_000 })
    .toBe(notificationReceiptCalls)
  await expect
    .poll(() => [...bridgeUpdateObserver.acceptedCalls], { timeout: 45_000 })
    .toEqual([
      expect.objectContaining({ method: "notify_withdrawal" }),
      expect.objectContaining({ method: "continue_withdrawal" }),
    ])
  ...
  await expect
    .poll(async () => BigInt((await controlState(request)).ledgerBalance))
    .toBe(BigInt(beforeWithdrawal.ledgerBalance) + 99_000_000n)
  await expect
    .poll(
      async () => {
        const state = await controlState(request)
...
async function openHistory(page: Page): Promise<void> {
  ...
  const progressOverlay = page
    .locator('div[data-state="open"][aria-hidden="true"].fixed.inset-0')
    .last()
...
async function holdIcUpdateMethod(
  page: Page,
  method: string,
): Promise<{ release: () => void; stop: () => Promise<void> }> {
  let release!: () => void
...
function decodeIcUpdateRequest(
  request: Request,
): { method: string; requestId: string } | undefined {
  const pathname = new URL(request.url()).pathname
  if (request.method() !== "POST" || !/^\/api\/v[234]\/canister\/[^/]+\/call$/.test(pathname))
    return undefined
  const body = request.postDataBuffer()
  if (!body) return { method: "<missing-body>", requestId: "<unknown>" }
  try {
    const envelope = Cbor.decode<{ content?: Record<string, unknown> & { method_name?: unknown } }>(
      new Uint8Array(body),
    )
    return {
      method:
        typeof envelope.content?.method_name === "string"
          ? envelope.content.method_name
          : "<missing-method>",
