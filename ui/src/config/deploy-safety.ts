...
export function assertTestUiProfile(profile: UiDeploymentMode): void {
  ...
  if (profile.chainId === BASE_MAINNET_CHAIN_ID) throw new Error("Test UI deploy rejects Base Mainnet")
  for (const canisterId of [profile.bridgeCanisterId, profile.ledgerCanisterId, profile.indexCanisterId]) {
...
export function assertProductionUiProfile(profile: UiDeploymentMode, verifiedManifestSha256?: string): void {
  ...
  if (profile.environmentMode !== null) throw new Error("Production UI deploy rejects test-only or unspecified environment modes")
