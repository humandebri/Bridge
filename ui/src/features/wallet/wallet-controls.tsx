...
export function visibleEvmConnectors(connectors: readonly Connector[]): Connector[] {
  ...
  const hasNamedInjected = supportedConnectors.some(
...
function isPlug(connector: Connector): boolean {
  return (
    connector.name.trim().toLowerCase() === "plug" ||
  ...
  return (
    <>
      <div className="flex items-center gap-1.5 sm:gap-2" aria-label="Wallet connections">
        <WalletSummary
          side="ic"
          value={ic.account?.owner}
          walletName={icBrand?.name}
          icon={icBrand?.icon}
          onClick={() => dialog.openFor("ic")}
        />
        <WalletSummary
          side="base"
          value={isConnected ? address : undefined}
          walletName={isConnected ? connector?.name : undefined}
          icon={connectorIcon(connector)}
          walletConnect={isConnected && connector ? isWalletConnect(connector) : false}
          onClick={() => dialog.openFor("base")}
        />
      </div>
      <WalletDialog />
    </>
...
function WalletSummary({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={connected ? `${label} connected as ${value}` : `Connect ${label}`}
    ...
    >
      ...
      <span className="hidden min-w-0 leading-none min-[360px]:block">
        <span className="block text-[9px] font-bold uppercase tracking-[.08em] text-[var(--muted)] sm:text-[10px]">
          {label}
        </span>
        <span className="mt-1 block max-w-[82px] truncate text-[11px] font-bold text-black sm:text-xs">
          {displayValue}
        </span>
      </span>
      {connected && (
        <span
          className="absolute right-1.5 top-1.5 size-2 rounded-full border-2 border-white bg-[var(--success)]"
          aria-hidden="true"
        />
      )}
    </button>
...
function WalletDialog() {
  ...
  const connectEvm = (nextConnector: Connector) =>
    void connectAsync({
  ...
  const addKinic = () => {
    if (
      ...
      !bsnsAddress ||
      !tokenChainMatches ||
      watchAsset.isPending ||
      addTokenInFlight.current
    )
      return
    addTokenInFlight.current = true
    void withBrowserLock(`kinic-wallet-prompt:base:${address.toLowerCase()}`, () =>
      ...
      .then((added) => {
        if (added) toast.success("KINIC added to wallet.")
          ...
          toast.info(
            `KINIC was not added. Add ${bsnsAddress} manually on ${deploymentProfile.label}.`,
          )
      })
      .catch(() => {
        toast.error(
          `This wallet could not add KINIC automatically. Add ${bsnsAddress} manually on ${deploymentProfile.label}.`,
        )
      })
      .finally(() => {
  ...
  return (
    <Dialog open={dialog.open} onOpenChange={(open) => dialog.setOpen(open)}>
      <DialogContent className="max-h-[min(720px,calc(100vh-2rem))] overflow-y-auto">
        <DialogHeader>
          <div className="mb-3 flex items-center gap-3">
            <span className="grid size-10 shrink-0 place-items-center overflow-hidden rounded-xl bg-white">
              <img
                src={headerLogo}
                alt=""
                data-dialog-network-logo={target}
                className={target === "ic" ? "w-9" : "size-10 object-cover"}
              />
            </span>
        ...
        <div className="mt-6 space-y-6">
          {(!target || target === "base") && (
            <WalletSection label="EVM wallet">
              {isConnected && address ? (
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 rounded-2xl border border-[var(--line)] bg-white p-3">
                    <span className="grid size-10 shrink-0 place-items-center overflow-hidden rounded-xl bg-[var(--panel)]">
                      <img
                        src="/kinic-token-logo.svg"
                        alt="KINIC token logo"
                ...
                <div className="grid gap-2">
                  {connectors.map((nextConnector) => (
                    <WalletOption
                      ...
                      name={
                        isGenericInjected(nextConnector) ? "Browser wallet" : nextConnector.name
                      }
                      description={
                        isWalletConnect(nextConnector)
                          ? "Scan with a mobile wallet"
                          : isCoinbaseWallet(nextConnector)
                            ? "Coinbase app or smart wallet"
                            : isMetaMask(nextConnector)
                              ? "Browser extension"
                              : isGenericInjected(nextConnector)
          ...
          {(!target || target === "ic") && (
            <WalletSection label="IC wallet">
              {ic.account && connectedBrand ? (
                <ConnectedWallet
                ...
                <div className="grid gap-2">
                  {(
                    Object.entries(icWalletBrands) as [
                      IcWalletProvider,
                      (typeof icWalletBrands)[IcWalletProvider],
                    ][]
                  ).map(([provider, brand]) => (
                    <WalletOption
                      key={provider}
...
function WalletSection({ label, children }: { label: string; children: ReactNode }) {
  return (
    <section aria-label={label}>
      <p className="mb-2 text-[10px] font-bold uppercase tracking-[.13em] text-[var(--muted)]">
        {label}
      </p>
      {children}
    </section>
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-0.5 block truncate text-xs text-[var(--muted)]">
          {busy ? "Waiting for wallet…" : description}
        </span>
      </span>
      {busy ? (
        <LoaderCircle
          className="size-4 shrink-0 animate-spin text-[var(--pink)]"
          aria-hidden="true"
        />
      ) : (
        <ArrowRight
          className="size-4 shrink-0 text-[var(--line-strong)] transition-transform group-hover:translate-x-0.5 group-hover:text-black"
          aria-hidden="true"
        />
      )}
    </button>
...
function ConnectedWallet({
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-[var(--panel)] p-3">
      <WalletGlyph side={side} name={name} icon={icon} walletConnect={walletConnect} />
      <div className="min-w-0 flex-1">
        <p className="flex items-center gap-2 text-sm font-bold text-black">
          {name}
          <Check className="size-4 text-[var(--success)]" aria-label="Connected" />
        </p>
        <p className="mt-0.5 truncate text-xs text-[var(--muted)]">{value}</p>
      </div>
      <Button variant="ghost" size="icon" aria-label={`Disconnect ${name}`} onClick={onDisconnect}>
...
function WalletEmpty() {
  return (
    <div className="rounded-2xl bg-[var(--panel)] px-4 py-5 text-center">
      ...
      <p className="mt-1 text-xs leading-5 text-[var(--muted)]">
        Install a browser wallet or enable WalletConnect for this environment.
      </p>
    </div>
  )
}
...
function WalletGlyph({
  ...
  if (icon)
    return (
      <span
        className={`${size} grid shrink-0 place-items-center overflow-hidden rounded-xl bg-white`}
      >
        <img
          src={icon}
          alt={`${name} logo`}
  ...
  if (walletConnect)
    return (
      <span
        className={`${size} grid shrink-0 place-items-center rounded-xl bg-[#3b99fc] text-white`}
      >
        <QrCode className={compact ? "size-4" : "size-5"} aria-hidden="true" />
      </span>
    )
  ...
  return (
    ...
    >
      <img
        src={side === "ic" ? icpLogo : baseLogo}
        alt={`${networkName} logo`}
        data-network-logo={side}
        className={
          side === "ic"
            ? `${compact ? "w-7" : "w-9"} h-auto`
            : `${compact ? "size-7" : "size-9"} object-contain`
        }
      />
    </span>
  )
