...
function connector(
  input: Partial<Connector> & Pick<Connector, "id" | "name" | "type" | "uid">,
): Connector {
...
describe("wallet controls", () => {
  ...
  it("orders named browser wallets before WalletConnect and removes the generic duplicate", () => {
    expect(
      visibleEvmConnectors([walletConnect, rabby, injected, metamask]).map((item) => item.name),
    ).toEqual(["MetaMask", "Rabby Wallet", "WalletConnect"])
    expect(visibleEvmConnectors([injected, walletConnect]).map((item) => item.name)).toEqual([
      "Injected",
      "WalletConnect",
    ])
  })
  ...
  it("never exposes Plug or a generic connector that may resolve to Plug on Base", () => {
    expect(
      visibleEvmConnectors([plugEvm, injected, metamask, walletConnect]).map((item) => item.name),
  ...
  it("renders detected EVM wallets as choices and connects the selected connector", () => {
    ...
    expect(
      within(section).queryByRole("button", { name: "Connect Browser wallet" }),
    ...
    expect(within(section).getByRole("img", { name: "Rabby Wallet logo" })).toHaveAttribute(
      "src",
      rabby.icon,
    )
  ...
  it("uses the connected provider logo and only the address in the EVM wallet summary", () => {
    ...
    expect(within(summary).getByRole("img", { name: "Rabby Wallet logo" })).toHaveAttribute(
      "src",
      rabby.icon,
    )
    ...
    expect(
      screen.getByRole("dialog").querySelector('[data-dialog-network-logo="base"]'),
    ).toBeVisible()
  })
  ...
  it("adds the profile-bound KINIC contract to the connected wallet", async () => {
    ...
    expect(screen.getByRole("img", { name: "KINIC token logo" })).toHaveAttribute(
      "src",
      "/kinic-token-logo.svg",
    )
    fireEvent.click(screen.getByRole("button", { name: "Add to wallet" }))

    await waitFor(() =>
      expect(mocks.watchAssetAsync).toHaveBeenCalledWith({
        type: "ERC20",
        options: {
          address: bsnsAddress,
          symbol: "KINIC",
  ...
  it("prevents duplicate requests before the pending state rerenders", async () => {
    ...
    mocks.watchAssetAsync.mockReturnValue(
      new Promise<boolean>((resolve) => {
        resolveWatchAsset = resolve
      }),
    )
    render(
      <WalletDialogProvider>
  ...
  it("explains wallet rejection and unsupported automatic token addition", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Add to wallet" }))
    await waitFor(() =>
      expect(mocks.toastInfo).toHaveBeenCalledWith(expect.stringContaining(bsnsAddress)),
    )
    fireEvent.click(screen.getByRole("button", { name: "Add to wallet" }))
    await waitFor(() =>
      expect(mocks.toastError).toHaveBeenCalledWith(expect.stringContaining(bsnsAddress)),
    )
  })
  ...
  it("uses the connected provider logo and only the principal in the IC wallet summary", () => {
    ...
    expect(
      screen.getByRole("dialog").querySelector('[data-dialog-network-logo="ic"]'),
    ).toBeVisible()
  })
})
