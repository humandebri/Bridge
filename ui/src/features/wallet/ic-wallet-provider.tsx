...
import {
  ...
  PlugAdapter,
  type IcAccount,
  type IcWalletAdapter,
  type IcWalletProvider,
} from "@/lib/ic/wallet"
...
export function IcWalletProviderRoot({ children }: { children: ReactNode }) {
  ...
  const connect = useCallback(
    async (nextProvider: IcWalletProvider) => {
      if (!deploymentProfile.ledgerCanisterId || !deploymentProfile.bridgeCanisterId)
        throw new Error("Bridge is temporarily unavailable")
...
function restoreWallet(owner: ReturnType<typeof loadIcHistoryOwner>):
  ...
  | undefined {
  if (!owner || !deploymentProfile.ledgerCanisterId || !deploymentProfile.bridgeCanisterId)
    return undefined
