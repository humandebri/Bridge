...
function toastSettlement(result: SettlementActionResult) { if ("Stopped" in result) { toast.error("This transfer needs attention. Try again from History."); return } if ("WaitingForConfirmation" in result || "Submitted" in result) { toast.info("Submitted. Check History after finalization if it has not completed."); return } if ("ReconciliationProgress" in result) { toast.info("Status updated. Processing will continue automatically."); return } toast.success(`Transfer ${settlementStateName(result.Complete.state).toLowerCase()}.`) }
