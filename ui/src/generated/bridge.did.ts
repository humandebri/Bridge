/* Generated by didc from canister/bridge-canister/bridge.did. Do not edit. */
import type { Principal } from '@dfinity/principal';
import type { ActorMethod } from '@dfinity/agent';
import type { IDL } from '@dfinity/candid';

export interface ActivationAttestation {
  'finalized_block_number' : bigint,
  'timelock_runtime_code_hash' : Uint8Array | number[],
  'base_service_fee' : bigint,
  'bsns_runtime_sha256' : Uint8Array | number[],
  'bsns_name' : string,
  'bsns_address' : Uint8Array | number[],
  'observed_at_ns' : bigint,
  'bsns_decimals' : number,
  'timelock_proposer' : Uint8Array | number[],
  'bsns_symbol' : string,
  'deposits_paused' : boolean,
  'bridge_runtime_sha256' : Uint8Array | number[],
  'timelock_executor' : Uint8Array | number[],
  'bridge_signer' : Uint8Array | number[],
  'chain_id' : bigint,
  'bridge_approved_timelock_runtime_code_hash' : Uint8Array | number[],
  'finalized_block_hash' : Uint8Array | number[],
  'bsns_bridge' : Uint8Array | number[],
  'timelock_minimum_delay_seconds' : bigint,
  'timelock_canceller' : Uint8Array | number[],
  'timelock_admin' : Uint8Array | number[],
  'runtime_administrator' : Uint8Array | number[],
  'withdrawals_paused' : boolean,
  'bridge_timelock' : Uint8Array | number[],
}
export interface ActivationConfirmationView {
  'transaction_hash' : Uint8Array | number[],
  'governance_operation_id' : bigint,
  'phase' : string,
  'timelock_operation_id' : Uint8Array | number[],
  'receipt_block_number' : bigint,
}
export interface ActivationOperationView {
  'salt' : Uint8Array | number[],
  'operation_id' : Uint8Array | number[],
}
export interface ActivationStatus {
  'pending_timelock_operation' : [] | [ActivationOperationView],
  'deposits_paused' : boolean,
  'last_confirmed_activation' : [] | [ActivationConfirmationView],
}
export type AdminError = { 'Busy' : null } |
  { 'InsufficientFeeReserve' : null } |
  { 'Unauthorized' : null } |
  { 'InvalidArgument' : string } |
  { 'StorageFailure' : null };
export interface AuditEvent {
  'timestamp_ns' : bigint,
  'kind' : AuditEventKind,
  'caller' : Principal,
  'sequence' : bigint,
}
export type AuditEventKind = { 'PausePrincipalRotated' : null } |
  { 'DepositsPauseRepeated' : null } |
  {
    'EvmRpcObservation' : {
      'finalized_block_number' : bigint,
      'transaction_hash' : [] | [Uint8Array | number[]],
      'call_method' : string,
      'evm_rpc_canister_id' : Principal,
      'finalized_block_hash' : Uint8Array | number[],
      'quorum_response_digest' : Uint8Array | number[],
      'request_digest' : Uint8Array | number[],
    }
  } |
  { 'WithdrawalFeeGuardCleared' : null } |
  { 'DepositsPaused' : null } |
  {
    'EvmRpcDecision' : {
      'bridge_operation_continued' : boolean,
      'configured_provider_count' : number,
      'transaction_hash' : [] | [Uint8Array | number[]],
      'kind' : string,
      'deposits_paused' : boolean,
      'automatically_resigned' : boolean,
      'required_threshold' : number,
      'operation' : string,
      'ledger_call_performed' : boolean,
      'stop_reason' : [] | [string],
    }
  } |
  {
    'FeeRecipientRotated' : {
      'previous_sha256' : Uint8Array | number[],
      'current_sha256' : Uint8Array | number[],
    }
  } |
  { 'DepositsResumed' : null } |
  { 'FeePayoutRequested' : { 'amount' : bigint } } |
  {
    'WithdrawalFeeGuardTripped' : {
      'charged_service_fee' : bigint,
      'ledger_fee' : bigint,
    }
  };
export interface AuditEventPage {
  'pruned_digest' : Uint8Array | number[],
  'oldest_available_sequence' : bigint,
  'events' : Array<AuditEvent>,
  'next_sequence' : [] | [bigint],
  'pruned_count' : bigint,
  'pruned_through_sequence' : [] | [bigint],
}
export type AutomaticProgressState = {
    'Scheduled' : { 'next_run_at_ns' : bigint }
  } |
  { 'Running' : { 'lease_until_ns' : bigint } };
export interface AutomaticProgressView { 'state' : AutomaticProgressState }
export type BaseGovernanceAction = { 'PauseDepositMints' : null } |
  { 'SetServiceFee' : { 'value' : bigint } } |
  { 'PauseWithdrawals' : null } |
  { 'CancelPendingTimelock' : null };
export interface BaseGovernanceConfirmation {
  'transaction_hash' : Uint8Array | number[],
  'operation_id' : bigint,
  'receipt_block_number' : bigint,
  'succeeded' : boolean,
}
export type BaseGovernanceError = {
    'InsufficientGovernanceBalance' : {
      'observed_wei' : bigint,
      'required_wei' : bigint,
    }
  } |
  { 'Busy' : { 'operation_id' : bigint } } |
  { 'TransactionNotFinalized' : { 'operation_id' : bigint } } |
  { 'Unauthorized' : null } |
  { 'InvalidArgument' : null } |
  { 'TransactionReverted' : { 'operation_id' : bigint } } |
  { 'StorageFailure' : null } |
  { 'ObservationUnavailable' : null } |
  { 'ReplacementLimitReached' : { 'operation_id' : bigint } } |
  { 'SigningUnavailable' : { 'class' : SigningFailureClass } };
export type BaseGovernanceOperationKind = { 'PauseDepositMints' : null } |
  { 'CancelTimelock' : { 'operation_id' : Uint8Array | number[] } } |
  { 'SetServiceFee' : { 'value' : bigint } } |
  { 'PauseWithdrawals' : null } |
  { 'ScheduleActivation' : ActivationOperationView } |
  { 'ExecuteActivation' : ActivationOperationView };
export interface BridgeInitArgs {
  'settlement_rate_limit_global' : number,
  'settlement_rate_limit_per_principal' : number,
  'settlement_cycle_ceiling' : bigint,
  'settlement_rate_limit_per_record' : number,
  'governance_evm_fee' : EvmFeePolicy,
  'notification_rate_limit_global' : number,
  'deposit_rate_limit_window_seconds' : bigint,
  'ecdsa_key_name' : string,
  'base_chain_id' : bigint,
  'bridge_contract' : Uint8Array | number[],
  'fee_recipient' : FeeRecipientConfig,
  'notification_rate_limit_window_seconds' : bigint,
  'settlement_rate_limit_window_seconds' : bigint,
  'ecdsa_derivation_path' : Array<Uint8Array | number[]>,
  'notification_ingestion_rate_limit_global' : number,
  'evm_rpc_canister_id' : Principal,
  'deposit_rate_limit_per_principal' : number,
  'deployment_instance_id' : Uint8Array | number[],
  'governance_ecdsa_derivation_path' : Array<Uint8Array | number[]>,
  'custom_evm_rpc_urls' : Array<string>,
  'minimum_withdrawal_id' : Uint8Array | number[],
  'deposit_rate_limit_global' : number,
  'pause_principal' : Principal,
  'governance_principal' : Principal,
  'index_canister_id' : Principal,
  'confirmation_relayer_principal' : Principal,
  'ledger_canister_id' : Principal,
  'settlement_retry_interval_seconds' : bigint,
  'governance_replacement' : GovernanceReplacementPolicy,
  'expected_bridge_runtime_sha256' : Uint8Array | number[],
  'timelock_contract' : Uint8Array | number[],
  'cycles_floor' : bigint,
}
export interface BridgeStatus {
  'unpaid_withdrawal_count' : bigint,
  'withdrawal_fee_guard_ledger_fee' : [] | [bigint],
  'mint_authorization_ttl_seconds' : bigint,
  'mint_authorization_epoch' : bigint,
  'unpaid_withdrawal_amount_out' : bigint,
  'reserve' : ReserveStatus,
  'withdrawal_stop_reasons' : Array<string>,
  'audit_retention_warning' : boolean,
  'deposits_paused' : boolean,
  'schema_version' : number,
  'observed_bridge_runtime_sha256' : Uint8Array | number[],
  'last_finalized_observation_ns' : bigint,
  'withdrawal_fee_guard_active' : boolean,
  'last_finalized_base_block' : bigint,
  'observed_bridge_signer' : Uint8Array | number[],
  'oldest_unpaid_withdrawal_observed_at_ns' : [] | [bigint],
  'counts' : StatusCounts,
  'withdrawal_fee_guard_charged_service_fee' : [] | [bigint],
  'settlement_scheduler' : SettlementSchedulerStatus,
  'last_finalized_base_block_hash' : Uint8Array | number[],
  'last_audit_sequence' : [] | [bigint],
}
export interface ChecksumRefreshStatus {
  'scanned_bytes' : bigint,
  'db_size' : bigint,
  'complete' : boolean,
  'checksum' : bigint,
}
export interface ConfirmBaseGovernanceTransactionArgs {
  'transaction_hash' : Uint8Array | number[],
  'operation_id' : bigint,
}
export interface DepositArgs {
  'base_recipient' : Uint8Array | number[],
  'max_service_fee' : bigint,
  'from_subaccount' : [] | [Uint8Array | number[]],
  'owner_sequence' : bigint,
  'gross_amount' : bigint,
}
export type DepositError = { 'Busy' : null } |
  { 'FundingRejected' : FundingFailure } |
  { 'BaseObservationUnavailable' : null } |
  { 'SequenceMismatch' : { 'expected' : bigint } } |
  { 'ReserveUnavailable' : null } |
  { 'DepositsPaused' : null } |
  { 'Rejected' : string } |
  { 'DepositIdentityConflict' : null } |
  { 'RateLimited' : { 'retry_after_seconds' : bigint } } |
  { 'InvalidRequest' : string } |
  { 'DepositConflict' : null } |
  { 'StorageFailure' : null } |
  { 'FundingUnavailable' : { 'retry_after_seconds' : bigint } } |
  { 'ReservationMaintenance' : { 'retry_after_seconds' : bigint } };
export interface DepositIdPage {
  'history_truncated' : boolean,
  'oldest_available_cursor' : [] | [bigint],
  'next_cursor' : [] | [bigint],
  'deposit_ids' : Array<Uint8Array | number[]>,
}
export type DepositPhase = { 'Refunded' : null } |
  { 'FundingReconciliationHold' : null } |
  { 'EscrowedUnquoted' : null } |
  { 'AuthorizationAvailable' : null } |
  { 'RefundProcessing' : null } |
  { 'Minted' : null } |
  { 'Cancelled' : null } |
  { 'AuthorizationPending' : null } |
  { 'RefundAvailable' : null };
export interface DepositQuoteView {
  'net_amount' : bigint,
  'service_fee' : bigint,
}
export interface DepositReceipt {
  'deposit_id' : Uint8Array | number[],
  'state' : DepositPhase,
  'owner_sequence' : bigint,
}
export type DepositRefundReasonView = { 'ServiceFeeRejected' : null } |
  { 'MintWindowLimitExceeded' : null } |
  { 'BasePaused' : null } |
  { 'AuthorizationExpired' : null } |
  { 'PerDepositLimitExceeded' : null };
export type DepositRefundStatusView = { 'Sending' : null } |
  { 'ReconciliationRequired' : null } |
  { 'Completed' : null };
export interface DepositRefundView {
  'status' : DepositRefundStatusView,
  'refund_ledger_block_index' : [] | [bigint],
  'attempt_no' : bigint,
  'ledger_fee' : bigint,
  'amount' : bigint,
  'reason' : DepositRefundReasonView,
}
export interface DepositView {
  'base_recipient' : Uint8Array | number[],
  'deposit_id' : Uint8Array | number[],
  'quote' : [] | [DepositQuoteView],
  'max_service_fee' : bigint,
  'funding_ledger_block_index' : [] | [bigint],
  'from_subaccount' : [] | [Uint8Array | number[]],
  'last_settlement_stop_reason' : [] | [string],
  'created_at_ns' : bigint,
  'state' : DepositPhase,
  'available_refund_amount' : [] | [bigint],
  'owner_sequence' : bigint,
  'mint_authorization' : [] | [MintAuthorizationView],
  'automatic_progress' : [] | [AutomaticProgressView],
  'gross_amount' : bigint,
  'refund' : [] | [DepositRefundView],
}
export interface EmergencyPauseReceipt {
  'local_pause_audit_sequence' : bigint,
  'base_action_count' : number,
  'caller' : Principal,
  'local_pause_audit_sha256' : Uint8Array | number[],
  'base_action_plan_sha256' : Uint8Array | number[],
  'local_deposits_paused' : boolean,
  'base_actions_queued' : boolean,
}
export interface EvmFeePolicy {
  'l1_fee_per_transaction_ceiling_wei' : bigint,
  'quote_validity_seconds' : bigint,
  'gas_limit_ceiling' : bigint,
  'max_priority_fee_per_gas_ceiling' : bigint,
  'l1_fee_multiplier_bps' : number,
  'gas_limit_multiplier_bps' : number,
  'base_fee_multiplier_bps' : number,
  'max_fee_per_gas_ceiling' : bigint,
}
export type FeePayoutActionResult = {
    'Stopped' : { 'state' : FeePayoutState, 'reason' : SettlementStopReason }
  } |
  { 'Complete' : { 'state' : FeePayoutState } } |
  { 'ReconciliationProgress' : { 'state' : FeePayoutState } };
export interface FeePayoutReceipt {
  'id' : bigint,
  'state' : FeePayoutState,
  'amount' : bigint,
}
export type FeePayoutState = { 'Failed' : null } |
  { 'Succeeded' : { 'block_index' : bigint } } |
  { 'ReconciliationHold' : null } |
  { 'Pending' : null };
export interface FeeRecipientConfig {
  'owner' : Principal,
  'subaccount' : Uint8Array | number[],
}
export type FundingFailure = {
    'InsufficientAllowance' : { 'allowance' : bigint }
  } |
  { 'BadBurn' : { 'minimum' : bigint } } |
  { 'BadFee' : { 'expected_fee' : bigint } } |
  { 'InsufficientFunds' : { 'balance' : bigint } };
export type GetWithdrawalsError = { 'TooManyIds' : null };
export interface GovernanceReplacementPolicy {
  'fee_bump_bps' : number,
  'max_replacements' : number,
}
export interface Icrc10SupportedStandard { 'url' : string, 'name' : string }
export interface Icrc21ConsentInfo {
  'metadata' : Icrc21ConsentMessageMetadata,
  'consent_message' : Icrc21ConsentMessage,
}
export type Icrc21ConsentMessage = { 'GenericDisplayMessage' : string };
export interface Icrc21ConsentMessageMetadata {
  'utc_offset_minutes' : [] | [number],
  'language' : string,
}
export interface Icrc21ConsentMessageRequest {
  'arg' : Uint8Array | number[],
  'method' : string,
  'user_preferences' : Icrc21ConsentMessageSpec,
}
export type Icrc21ConsentMessageResponse = { 'Ok' : Icrc21ConsentInfo } |
  { 'Err' : Icrc21Error };
export interface Icrc21ConsentMessageSpec {
  'metadata' : Icrc21ConsentMessageMetadata,
  'device_spec' : [] | [Icrc21DeviceSpec],
}
export type Icrc21DeviceSpec = { 'GenericDisplay' : null } |
  { 'FieldsDisplay' : null };
export type Icrc21Error = { 'GenericError' : Icrc21GenericError } |
  { 'InsufficientPayment' : Icrc21ErrorInfo } |
  { 'UnsupportedCanisterCall' : Icrc21ErrorInfo } |
  { 'ConsentMessageUnavailable' : Icrc21ErrorInfo };
export interface Icrc21ErrorInfo { 'description' : string }
export interface Icrc21GenericError {
  'description' : string,
  'error_code' : bigint,
}
export interface ListDepositIdsArgs {
  'owner' : Principal,
  'limit' : number,
  'before_cursor' : [] | [bigint],
}
export type ListDepositIdsError = { 'InvalidLimit' : null };
export interface MintAuthorizationView {
  'finalized_block_number' : bigint,
  'signature' : [] | [Uint8Array | number[]],
  'deposit_id' : Uint8Array | number[],
  'domain_name' : string,
  'charged_service_fee' : bigint,
  'recipient' : Uint8Array | number[],
  'domain_version' : string,
  'authorization_epoch' : bigint,
  'max_service_fee' : bigint,
  'deadline' : bigint,
  'signature_dispatch_attempt' : number,
  'chain_id' : bigint,
  'finalized_block_hash' : Uint8Array | number[],
  'finalized_block_timestamp' : bigint,
  'verifying_contract' : Uint8Array | number[],
  'digest' : Uint8Array | number[],
  'gross_amount' : bigint,
}
export interface NonterminalDepositRef {
  'deposit_id' : Uint8Array | number[],
  'owner_sequence' : bigint,
}
export interface NonterminalDepositRefPage {
  'next_cursor' : [] | [bigint],
  'deposits' : Array<NonterminalDepositRef>,
}
export interface NotifyWithdrawalArgs {
  'transaction_hash' : Uint8Array | number[],
}
export type NotifyWithdrawalError = {
    'LedgerFeeExceedsServiceFee' : {
      'charged_service_fee' : bigint,
      'ledger_fee' : bigint,
    }
  } |
  { 'Busy' : null } |
  { 'RpcUnavailable' : null } |
  { 'TransactionNotConfirmed' : null } |
  { 'InsufficientCycles' : null } |
  { 'WithdrawalConflict' : null } |
  { 'RpcInconsistent' : null } |
  { 'RateLimited' : null } |
  { 'InvalidTransactionHash' : null } |
  { 'TransactionReverted' : null } |
  {
    'WithdrawalBeforeAdmissionBoundary' : {
      'observed_withdrawal_id' : Uint8Array | number[],
      'minimum_withdrawal_id' : Uint8Array | number[],
    }
  } |
  { 'StorageFailure' : null } |
  { 'BaseStateMismatch' : null } |
  { 'TransactionNotFound' : null } |
  { 'BridgeSignerMismatch' : null } |
  { 'AnonymousCaller' : null } |
  { 'InvalidBaseResponse' : null };
export type NotifyWithdrawalReceipt = {
    'Duplicate' : { 'withdrawal_id' : Uint8Array | number[] }
  } |
  {
    'Ingested' : {
      'withdrawal_id' : Uint8Array | number[],
      'finalized_checkpoint_block_number' : bigint,
    }
  };
export interface OperationalConfigArgs {
  'settlement_cycle_ceiling' : bigint,
  'governance_evm_fee' : EvmFeePolicy,
  'cycles_floor' : bigint,
}
export interface OperationalConfigSealReceipt {
  'activation_attestation' : ActivationAttestation,
  'lifecycle' : ProductionLifecycle,
}
export interface PrepareBaseGovernanceReplacementArgs {
  'max_priority_fee_per_gas' : bigint,
  'operation_id' : bigint,
  'max_fee_per_gas' : bigint,
  'expected_transaction_hash' : Uint8Array | number[],
}
export type ProductionLifecycle = { 'Bootstrap' : null } |
  { 'Activated' : null } |
  { 'OperationalConfigSealed' : null };
export interface PublicConfig {
  'settlement_rate_limit_global' : number,
  'expected_bridge_signer' : Uint8Array | number[],
  'settlement_rate_limit_per_principal' : number,
  'settlement_cycle_ceiling' : bigint,
  'settlement_rate_limit_per_record' : number,
  'mint_authorization_ttl_seconds' : bigint,
  'governance_evm_fee' : EvmFeePolicy,
  'mint_authorization_epoch' : bigint,
  'notification_rate_limit_global' : number,
  'deposit_rate_limit_window_seconds' : bigint,
  'base_chain_id' : bigint,
  'bridge_contract' : Uint8Array | number[],
  'fee_recipient' : FeeRecipientConfig,
  'notification_rate_limit_window_seconds' : bigint,
  'settlement_rate_limit_window_seconds' : bigint,
  'notification_ingestion_rate_limit_global' : number,
  'evm_rpc_canister_id' : Principal,
  'deposit_rate_limit_per_principal' : number,
  'deployment_instance_id' : Uint8Array | number[],
  'schema_version' : number,
  'minimum_withdrawal_id' : Uint8Array | number[],
  'deposit_rate_limit_global' : number,
  'pause_principal' : Principal,
  'governance_principal' : Principal,
  'ledger_fee' : bigint,
  'index_canister_id' : Principal,
  'confirmation_relayer_principal' : Principal,
  'ledger_canister_id' : Principal,
  'settlement_retry_interval_seconds' : bigint,
  'governance_replacement' : GovernanceReplacementPolicy,
  'governance_operator' : Uint8Array | number[],
  'expected_bridge_runtime_sha256' : Uint8Array | number[],
  'timelock_contract' : Uint8Array | number[],
  'cycles_floor' : bigint,
  'rpc_provider_urls_sha256' : Uint8Array | number[],
}
export type PublicConfigInitializationError = { 'ConflictingAddress' : null } |
  { 'Busy' : null } |
  { 'Unauthorized' : null } |
  { 'StorageFailure' : null } |
  { 'DerivationUnavailable' : null };
export type RequestDepositRefundError = {
    'AutomaticProgressPending' : { 'next_run_at_ns' : [] | [bigint] }
  } |
  { 'NotClaimable' : null } |
  { 'Busy' : null } |
  { 'InvalidDepositId' : null } |
  { 'NotFound' : null } |
  { 'InsufficientCycles' : null } |
  { 'DepositIdentityConflict' : null } |
  { 'RpcInconsistent' : null } |
  { 'RateLimited' : { 'retry_after_seconds' : bigint } } |
  { 'StorageFailure' : null } |
  { 'BaseStateMismatch' : null } |
  { 'FinalityUnavailable' : null } |
  { 'AnonymousCaller' : null };
export interface ReserveStatus {
  'cycles_balance' : bigint,
  'cycles_surplus' : bigint,
  'sufficient' : boolean,
  'required_cycles' : bigint,
}
export type Result = { 'Ok' : BaseGovernanceConfirmation } |
  { 'Err' : BaseGovernanceError };
export type Result_1 = { 'Ok' : FeePayoutActionResult } |
  { 'Err' : SettlementActionError };
export type Result_10 = { 'Ok' : ProductionLifecycle } |
  { 'Err' : BaseGovernanceError };
export type Result_11 = { 'Ok' : Array<[] | [WithdrawalView]> } |
  { 'Err' : GetWithdrawalsError };
export type Result_12 = { 'Ok' : null } |
  { 'Err' : PublicConfigInitializationError };
export type Result_13 = { 'Ok' : DepositIdPage } |
  { 'Err' : ListDepositIdsError };
export type Result_14 = { 'Ok' : NonterminalDepositRefPage } |
  { 'Err' : ListDepositIdsError };
export type Result_15 = { 'Ok' : NotifyWithdrawalReceipt } |
  { 'Err' : NotifyWithdrawalError };
export type Result_16 = { 'Ok' : null } |
  { 'Err' : AdminError };
export type Result_17 = { 'Ok' : ChecksumRefreshStatus } |
  { 'Err' : StorageMaintenanceError };
export type Result_18 = { 'Ok' : DepositReceipt } |
  { 'Err' : DepositError };
export type Result_19 = { 'Ok' : DepositView } |
  { 'Err' : RequestDepositRefundError };
export type Result_2 = { 'Ok' : StorageValidationStatus } |
  { 'Err' : StorageMaintenanceError };
export type Result_20 = { 'Ok' : FeePayoutReceipt } |
  { 'Err' : AdminError };
export type Result_21 = { 'Ok' : OperationalConfigSealReceipt } |
  { 'Err' : BaseGovernanceError };
export type Result_22 = { 'Ok' : string } |
  { 'Err' : StorageMaintenanceError };
export type Result_3 = { 'Ok' : SettlementActionResult } |
  { 'Err' : SettlementActionError };
export type Result_4 = { 'Ok' : EmergencyPauseReceipt } |
  { 'Err' : BaseGovernanceError };
export type Result_5 = { 'Ok' : SignedBaseGovernanceTransaction } |
  { 'Err' : BaseGovernanceError };
export type Result_6 = { 'Ok' : ActivationAttestation } |
  { 'Err' : BaseGovernanceError };
export type Result_7 = { 'Ok' : ActivationStatus } |
  { 'Err' : BaseGovernanceError };
export type Result_8 = { 'Ok' : AuditEventPage } |
  { 'Err' : AdminError };
export type Result_9 = { 'Ok' : [] | [SignedBaseGovernanceTransaction] } |
  { 'Err' : BaseGovernanceError };
export interface RotatePausePrincipalArgs { 'pause_principal' : Principal }
export type SettlementActionError = {
    'AutomaticProgressPending' : { 'next_run_at_ns' : [] | [bigint] }
  } |
  { 'InvalidId' : null } |
  { 'Busy' : null } |
  { 'WrongState' : null } |
  { 'NotFound' : null } |
  { 'InsufficientCycles' : null } |
  { 'Unauthorized' : null } |
  { 'RateLimited' : { 'retry_after_seconds' : bigint } } |
  { 'StorageFailure' : null } |
  { 'AnonymousCaller' : null };
export type SettlementActionResult = {
    'Stopped' : { 'state' : SettlementState, 'reason' : SettlementStopReason }
  } |
  { 'Complete' : { 'state' : SettlementState } } |
  { 'ReconciliationProgress' : { 'state' : SettlementState } } |
  { 'Deferred' : { 'state' : SettlementState, 'next_run_at_ns' : bigint } };
export type SettlementSchedulerHealth = { 'Healthy' : null } |
  { 'Degraded' : null } |
  { 'Faulted' : null };
export interface SettlementSchedulerStatus {
  'last_dispatcher_run_at_ns' : bigint,
  'scheduled' : bigint,
  'expired' : bigint,
  'next_wakeup_at_ns' : [] | [bigint],
  'stopped' : bigint,
  'leased' : bigint,
  'last_internal_error' : [] | [string],
  'health' : SettlementSchedulerHealth,
}
export type SettlementState = { 'Deposit' : DepositPhase } |
  { 'Withdrawal' : WithdrawalPhase };
export type SettlementStopReason = { 'LedgerFeeExceedsServiceFee' : null } |
  { 'LedgerRejected' : string } |
  { 'RpcUnavailable' : null } |
  { 'RpcInconsistent' : null } |
  { 'LedgerAmbiguous' : null } |
  { 'LedgerUnavailable' : null } |
  { 'AuthorizationExpired' : null } |
  { 'BaseStateMismatch' : null } |
  { 'BridgeSignerMismatch' : null } |
  { 'SigningUnavailable' : null } |
  { 'InvalidBaseResponse' : null };
export interface SignedBaseGovernanceTransaction {
  'transaction_hash' : Uint8Array | number[],
  'raw_transaction' : Uint8Array | number[],
  'max_priority_fee_per_gas' : bigint,
  'kind' : BaseGovernanceOperationKind,
  'generation' : number,
  'signed_at_ns' : bigint,
  'operation_id' : bigint,
  'max_fee_per_gas' : bigint,
  'sender' : Uint8Array | number[],
  'calldata' : Uint8Array | number[],
  'target' : Uint8Array | number[],
  'chain_id' : bigint,
  'nonce' : bigint,
  'gas_limit' : bigint,
}
export type SigningFailureClass = { 'CallFailed' : null } |
  { 'Storage' : null } |
  { 'InvalidSignature' : null } |
  { 'CostUnavailable' : null } |
  { 'InsufficientCycles' : null } |
  { 'ResponseDecode' : null } |
  { 'InvalidPublicKey' : null } |
  { 'CallRejected' : null } |
  { 'RecoveryMismatch' : null };
export interface StatusCounts {
  'retained_audit_events' : bigint,
  'reconciliation_holds' : bigint,
  'retained_deposit_index_entries' : bigint,
  'pending_ledger_operations' : bigint,
  'withdrawals' : bigint,
  'deposits' : bigint,
  'reserved_deposit_mint_operations' : bigint,
  'reserved_deposit_mint_amount' : bigint,
  'pruned_audit_events' : bigint,
}
export type StorageMaintenanceError = { 'StateChanged' : null } |
  { 'Unauthorized' : null } |
  { 'InvalidArgument' : { 'message' : string } } |
  { 'StorageFailure' : null } |
  { 'NotStarted' : null };
export interface StorageValidationStatus {
  'complete' : boolean,
  'phase' : string,
  'scanned_rows' : bigint,
}
export type WithdrawalPhase = { 'Paid' : null } |
  { 'ReleasePending' : null } |
  { 'ReconciliationHold' : null } |
  { 'Observed' : null };
export interface WithdrawalView {
  'charged_service_fee' : bigint,
  'withdrawal_id' : Uint8Array | number[],
  'max_service_fee' : bigint,
  'release_ledger_block_index' : [] | [bigint],
  'last_settlement_stop_reason' : [] | [string],
  'amount_out' : bigint,
  'state' : WithdrawalPhase,
  'ledger_fee' : bigint,
  'amount' : bigint,
}
export interface _SERVICE {
  'confirm_base_governance_transaction' : ActorMethod<
    [ConfirmBaseGovernanceTransactionArgs],
    Result
  >,
  'continue_fee_payout' : ActorMethod<[bigint], Result_1>,
  'continue_storage_validation' : ActorMethod<[number], Result_2>,
  'continue_withdrawal' : ActorMethod<[Uint8Array | number[]], Result_3>,
  'emergency_pause' : ActorMethod<[], Result_4>,
  'execute_activation' : ActorMethod<[], Result_5>,
  'get_activation_attestation' : ActorMethod<[], Result_6>,
  'get_activation_status' : ActorMethod<[], Result_7>,
  'get_audit_events' : ActorMethod<[bigint, number], Result_8>,
  'get_bridge_status' : ActorMethod<[], BridgeStatus>,
  'get_deposit' : ActorMethod<[Uint8Array | number[]], [] | [DepositView]>,
  'get_deposit_by_owner_sequence' : ActorMethod<
    [Principal, bigint],
    [] | [DepositView]
  >,
  'get_next_deposit_sequence' : ActorMethod<[Principal], bigint>,
  'get_pending_base_governance_transaction' : ActorMethod<[], Result_9>,
  'get_production_lifecycle' : ActorMethod<[], Result_10>,
  'get_public_config' : ActorMethod<[], PublicConfig>,
  'get_withdrawal' : ActorMethod<
    [Uint8Array | number[]],
    [] | [WithdrawalView]
  >,
  'get_withdrawals' : ActorMethod<[Array<Uint8Array | number[]>], Result_11>,
  'icrc10_supported_standards' : ActorMethod<
    [],
    Array<Icrc10SupportedStandard>
  >,
  'icrc21_canister_call_consent_message' : ActorMethod<
    [Icrc21ConsentMessageRequest],
    Icrc21ConsentMessageResponse
  >,
  'initialize_public_config' : ActorMethod<[], Result_12>,
  'list_deposit_ids' : ActorMethod<[ListDepositIdsArgs], Result_13>,
  'list_nonterminal_deposit_refs' : ActorMethod<
    [ListDepositIdsArgs],
    Result_14
  >,
  'notify_withdrawal' : ActorMethod<[NotifyWithdrawalArgs], Result_15>,
  'pause_new_deposits' : ActorMethod<[], Result_16>,
  'prepare_base_governance_action' : ActorMethod<
    [BaseGovernanceAction],
    Result_5
  >,
  'prepare_base_governance_replacement' : ActorMethod<
    [PrepareBaseGovernanceReplacementArgs],
    Result_5
  >,
  'prepare_next_emergency_base_action' : ActorMethod<[], Result_5>,
  'refresh_activation_attestation' : ActorMethod<[], Result_6>,
  'refresh_storage_checksum' : ActorMethod<[bigint], Result_17>,
  'request_deposit' : ActorMethod<[DepositArgs], Result_18>,
  'request_deposit_refund' : ActorMethod<[Uint8Array | number[]], Result_19>,
  'request_fee_payout' : ActorMethod<[bigint], Result_20>,
  'rotate_fee_recipient' : ActorMethod<[FeeRecipientConfig], Result_16>,
  'rotate_pause_principal' : ActorMethod<[RotatePausePrincipalArgs], Result_16>,
  'schedule_activation' : ActorMethod<[], Result_5>,
  'seal_operational_config' : ActorMethod<[OperationalConfigArgs], Result_21>,
  'start_storage_validation' : ActorMethod<[], Result_2>,
  'storage_integrity_check' : ActorMethod<[], Result_22>,
}
export declare const idlFactory: IDL.InterfaceFactory;
export declare const init: (args: { IDL: typeof IDL }) => IDL.Type[];
