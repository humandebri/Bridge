...
describe("bridgeAvailability", () => {
  it("marks both asset directions unavailable when the cycles floor is insufficient", () => {
    ...
    })).toEqual({ status: "Unavailable", available: false, toBase: "Unavailable", toIc: "Unavailable" })
  })
  ...
  it("fails closed when any observation is older than 60 seconds", () => {
    ...
    expect(statusDataIsFresh({ baseUpdatedAt: now - STATUS_FRESHNESS_MS - 1, canisterUpdatedAt: now, now })).toBe(false)
