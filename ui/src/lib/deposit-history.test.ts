...
describe("deposit history pagination", () => {
  it("preserves the deepest cursor on refresh and appends older pages without duplicates", () => {
    const first = mergeDepositHistoryPage(
      undefined,
      [deposit(5), deposit(4)],
      { nextCursor: 3n, oldestAvailableCursor: 1n, historyTruncated: false },
      "refresh",
    )
    const older = mergeDepositHistoryPage(
      first,
      [deposit(3), deposit(4)],
      { nextCursor: 2n, oldestAvailableCursor: 1n, historyTruncated: true },
      "older",
    )
    const refreshed = mergeDepositHistoryPage(
      older,
      [deposit(6), deposit(5)],
      { nextCursor: 4n, oldestAvailableCursor: 1n, historyTruncated: false },
      "refresh",
    )
  ...
  it("replaces a cached deposit with its refreshed state", () => {
    ...
    const cached = mergeDepositHistoryPage(
      undefined,
      [pending],
      { nextCursor: null, oldestAvailableCursor: 1n, historyTruncated: false },
      "refresh",
    )
  ...
  it("refreshes only the newest page and explicitly selected nonterminal cached deposits", () => {
    const cached = mergeDepositHistoryPage(
      undefined,
      Array.from({ length: 21 }, (_, index) => deposit(21 - index)),
      { nextCursor: null, oldestAvailableCursor: 1n, historyTruncated: false },
      "refresh",
    )
