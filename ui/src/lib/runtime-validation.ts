...
export function runtimeWriteBlocker(
  ...
  if (
    !Number.isFinite(validation.checkedAt) ||
    validation.checkedAt > now ||
    now - validation.checkedAt > RUNTIME_VALIDATION_TTL_MS
  ) {
...
export function requireRuntimeWriteReady(
  validation?: RuntimeValidation,
  now = Date.now(),
): asserts validation is RuntimeValidation & { ready: true } {
...
export async function refetchRuntimeWriteReady<T extends RuntimeValidation>(
  ...
  if (result.isError || result.error) {
    throw result.error instanceof Error
      ? result.error
      : new Error("Runtime verification could not be refreshed")
  }
...
export async function validateRuntimeHeartbeat(
  ...
  if (connectedChainId !== undefined && connectedChainId !== profile.chainId)
    blockers.push(`Wallet is on chain ${connectedChainId}; expected ${profile.chainId}`)
  ...
  if (localFinalized.number === null || localFinalized.hash === null)
    blockers.push("Finalized Base block number or hash is unavailable")
  if (blockers.length > 0) {
    return {
      ready: false,
      blockers,
...
export async function validateRuntime(
  ...
  if (expectedTimelockDelaySeconds === null) {
    return {
      ready: false,
      blockers: ["Timelock delay is missing"],
      checkedAt: Date.now(),
      profileFingerprint,
    }
  }
  ...
  if (!bridgeCode || runtimeBytecodeSha256(bridgeCode) !== profile.bridgeRuntimeHash)
    blockers.push("Bridge runtime bytecode does not match the reviewed profile")
  if (!bsnsCode || runtimeBytecodeSha256(bsnsCode) !== profile.bsnsRuntimeHash)
    blockers.push("bSNS runtime bytecode does not match the reviewed profile")
  if (String(linkedBsns).toLowerCase() !== bsnsAddress.toLowerCase())
    blockers.push("Bridge points to a different bSNS contract")
  if (
    bsnsSymbol !== profile.baseToken.symbol ||
    Number(bsnsDecimals) !== profile.baseToken.decimals
  ) {
    blockers.push(
      `Base token metadata is not ${profile.baseToken.symbol}/${profile.baseToken.decimals}`,
    )
  }
  ...
  blockers.push(
    ...bridgeSignerBlockers(
      profile.expected_bridge_signer as Address,
      bridgeSnapshot.bridgeSigner,
      config.expected_bridge_signer,
    ),
  )
  if (config.base_chain_id !== BigInt(profile.chainId))
    blockers.push("Canister Base chain ID differs from the profile")
  const configuredBridge = `0x${Array.from(config.bridge_contract, (byte) => Number(byte).toString(16).padStart(2, "0")).join("")}`
  if (configuredBridge.toLowerCase() !== bridgeAddress.toLowerCase())
    blockers.push("Canister Bridge contract differs from the profile")
  ...
  if (configuredTimelock.toLowerCase() !== timelockAddress.toLowerCase())
    blockers.push("Canister Timelock contract differs from the profile")
  ...
  if (configuredDeploymentInstance.toLowerCase() !== profile.deploymentInstanceId?.toLowerCase())
    blockers.push("Canister deployment instance differs from the profile")
  ...
  if (configuredMinimumWithdrawalId.toLowerCase() !== profile.minimumWithdrawalId?.toLowerCase())
    blockers.push("Canister minimum withdrawal ID differs from the profile")
  if (config.ledger_canister_id.toText() !== profile.ledgerCanisterId)
    blockers.push("Canister ledger differs from the profile")
  if (config.index_canister_id.toText() !== profile.indexCanisterId)
    blockers.push("Canister index differs from the profile")
  if (config.evm_rpc_canister_id.toText() !== profile.evmRpcCanisterId)
    blockers.push("Canister EVM RPC ID differs from the profile")
  ...
  if (
    ...
    ledgerSymbol !== profile.icToken.symbol ||
    ledgerDecimals !== profile.icToken.decimals
  ) {
    blockers.push(
      `IC token metadata is not ${profile.icToken.name}/${profile.icToken.symbol}/${profile.icToken.decimals}`,
    )
  }
  return {
    ready: blockers.length === 0,
...
export function bridgeSignerBlockers(
  profileSigner: Address,
  contractSigner: Address,
  canisterSigner?: Uint8Array | number[],
): string[] {
  if (!canisterSigner || canisterSigner.length !== 20)
    return ["Canister expected Bridge signer is unavailable"]
  ...
  if (contractSigner.toLowerCase() !== expected)
    blockers.push("Bridge signer differs from the reviewed profile")
