...
import {
  ...
  scanWithdrawalLogs,
  WITHDRAWAL_LOG_CHUNK_SIZE,
  WITHDRAWAL_SCAN_CHUNKS_PER_STEP,
...
const log = (id: number, blockNumber: bigint): TestLog => ({
  id,
  blockNumber,
  transactionHash: `0x${id.toString(16)}`,
  logIndex: 0,
})
...
describe("withdrawal log scanning", () => {
  ...
  it("keeps the older cursor when a bounded scan finds 20 events", async () => {
    const fetchLogs = vi.fn((fromBlock: bigint, toBlock: bigint) =>
      Promise.resolve(
        fromBlock === 48_002n
    ...
    const result = await scanWithdrawalLogs({
      deploymentBlock: 1n,
      finalizedBlock: 50_001n,
      finalizedBlockHash: "0x5001",
      fetchLogs,
    })
    ...
    expect(result.olderCursor).toBe(
      50_001n - WITHDRAWAL_LOG_CHUNK_SIZE * BigInt(WITHDRAWAL_SCAN_CHUNKS_PER_STEP),
    )
  })
  ...
  it("uses only newly confirmed ranges on refresh and deduplicates events", async () => {
    ...
    const result = await scanWithdrawalLogs({
      ...
      previous: {
        lastFinalizedBlock: 100n,
        lastFinalizedBlockHash: "0x0100",
        olderCursor: null,
  ...
  it("keeps every incremental RPC request within 2,000 blocks", async () => {
    ...
    await scanWithdrawalLogs({
      ...
      previous: {
        lastFinalizedBlock: 1n,
        lastFinalizedBlockHash: "0x0001",
        olderCursor: null,
        reachedDeploymentBlock: true,
        logs: [] as TestLog[],
      },
  ...
  it("caps each scan step and resumes older ranges from its cursor", async () => {
    ...
    const first = await scanWithdrawalLogs({
      deploymentBlock: 1n,
      finalizedBlock,
      finalizedBlockHash: "0x50000",
      fetchLogs,
    })
    expect(fetchLogs).toHaveBeenCalledTimes(WITHDRAWAL_SCAN_CHUNKS_PER_STEP)
    expect(first.olderCursor).toBe(
      finalizedBlock - WITHDRAWAL_LOG_CHUNK_SIZE * BigInt(WITHDRAWAL_SCAN_CHUNKS_PER_STEP),
    )
    expect(first.reachedDeploymentBlock).toBe(false)

    fetchLogs.mockClear()
    const second = await scanWithdrawalLogs({
      deploymentBlock: 1n,
      finalizedBlock,
      finalizedBlockHash: "0x50000",
      previous: first,
    ...
    expect(fetchLogs).toHaveBeenCalledTimes(WITHDRAWAL_SCAN_CHUNKS_PER_STEP)
    expect(second.olderCursor).toBe(
      first.olderCursor! - WITHDRAWAL_LOG_CHUNK_SIZE * BigInt(WITHDRAWAL_SCAN_CHUNKS_PER_STEP),
    )
  })
  ...
  it("caps a large finalized catch-up and resumes it on the next refresh", async () => {
    ...
    const previous = {
      lastFinalizedBlock: 1n,
      lastFinalizedBlockHash: "0x0001" as const,
      olderCursor: null,
      reachedDeploymentBlock: true,
      logs: [] as TestLog[],
    }
    ...
    expect(fetchLogs).toHaveBeenCalledTimes(WITHDRAWAL_SCAN_CHUNKS_PER_STEP)
    expect(first.lastFinalizedBlock).toBe(
      1n + WITHDRAWAL_LOG_CHUNK_SIZE * BigInt(WITHDRAWAL_SCAN_CHUNKS_PER_STEP),
    )
    ...
    expect(fetchLogs).toHaveBeenCalledTimes(WITHDRAWAL_SCAN_CHUNKS_PER_STEP)
    expect(second.lastFinalizedBlock).toBe(
      first.lastFinalizedBlock +
        WITHDRAWAL_LOG_CHUNK_SIZE * BigInt(WITHDRAWAL_SCAN_CHUNKS_PER_STEP),
    )
  })
  ...
  it("preserves the older cursor when a finalized refresh fills the visible history", async () => {
    const previous = {
      lastFinalizedBlock: 100n,
      lastFinalizedBlockHash: "0x0100" as const,
      olderCursor: 50n,
    ...
    const result = await scanWithdrawalLogs({
      ...
      previous,
      fetchLogs: () =>
        Promise.resolve(Array.from({ length: 20 }, (_, index) => log(index + 1, 101n))),
    })
  ...
  it("continues scanning after more than 20 events have already been found", async () => {
    ...
    expect(result.olderCursor).toBe(
      previous.olderCursor - WITHDRAWAL_LOG_CHUNK_SIZE * BigInt(WITHDRAWAL_SCAN_CHUNKS_PER_STEP),
    )
  })
})
