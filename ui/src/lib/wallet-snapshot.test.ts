...
describe("wallet snapshots", () => {
  it("reads the injected account and chain at signing time", async () => {
    Object.defineProperty(window, "ethereum", { configurable: true, value: { request: ({ method }: { method: string }) => Promise.resolve(method === "eth_accounts" ? ["0x1111111111111111111111111111111111111111"] : "0x14a34") } })
    await expect(currentInjectedWallet()).resolves.toEqual({ address: "0x1111111111111111111111111111111111111111", chainId: 84532 })
  })

  it("fails closed for a disconnected or malformed injected wallet", async () => {
    Object.defineProperty(window, "ethereum", { configurable: true, value: { request: () => Promise.resolve([]) } })
