...
describe("browser lock fallback", () => {
  it("holds a localStorage lease while serializing work without Web Locks", async () => {
    ...
    await entered
    expect(
      Object.keys(window.localStorage).some((key) => key.includes("browser-lock.v1:fallback-test")),
    ).toBe(true)

    const second = withBrowserLock("fallback-test", () => {
      order.push("second")
    })
  ...
  it("fails closed for wallet work whenever Web Locks are unavailable", async () => {
    ...
    await expect(withBrowserLock("kinic-wallet-prompt:ic:owner", action)).rejects.toThrow(
