...
PY
bash "$ROOT/scripts/current_main_concretize_bridge_runtime.sh" "$ROOT" "$T/profile.json" "$T/first.bin"
bash "$ROOT/scripts/current_main_concretize_bridge_runtime.sh" "$ROOT" "$T/profile.json" "$T/second.bin"
...
PY
if bash "$ROOT/scripts/current_main_concretize_bridge_runtime.sh" "$ROOT" "$T/profile.json" "$T/rejected.bin" >/dev/null 2>&1; then
