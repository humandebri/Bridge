...
evm_liveness=one('evm_liveness'); evm_fee=one('evm_fee'); fee_recipient=one('fee_recipient')
if not isinstance(evm_liveness,dict) or not isinstance(evm_fee,dict) or not isinstance(fee_recipient,dict): raise SystemExit('Canister public config nested values are malformed')
...
public_config={
 ...
 'evm_fee':{k:(str(num(v)) if k in ('gas_limit_ceiling','max_fee_per_gas_ceiling','max_priority_fee_per_gas_ceiling','l1_fee_per_transaction_ceiling_wei') else num(v)) for k,v in evm_fee.items()},
 'evm_liveness':{k:num(v) for k,v in evm_liveness.items()},
