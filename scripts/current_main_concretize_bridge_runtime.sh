...
FOUNDRY_PROFILE=default FOUNDRY_OFFLINE=true forge create --offline --broadcast --unlocked \
  ...
  "$PER_DEPOSIT" "$MINT_LIMIT" "$MINT_WINDOW" "$MAX_FEE" "$SERVICE_FEE" >/dev/null
