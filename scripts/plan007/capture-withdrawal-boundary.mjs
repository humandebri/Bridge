...
export async function collectWithdrawalBoundary(profile, config, fetchImpl = fetch) {
  ...
  const heads = await Promise.all(config.rpc_urls.map(async (url) => {
    ...
    return { number: quantity(block.number, "finalized block number"), hash: String(block.hash).toLowerCase() }
  }))
  ...
  const observations = (await Promise.all(config.rpc_urls.map(async (url, index) => {
    ...
    const [pausedWord, nextWord] = await Promise.all([
      rpc(url, "eth_call", [{ to: bridge, data: WITHDRAWALS_PAUSED_SELECTOR }, selector], fetchImpl),
      rpc(url, "eth_call", [{ to: bridge, data: NEXT_WITHDRAWAL_ID_SELECTOR }, selector], fetchImpl),
    ])
