...
`verification/claim-test-manifest.tsv` は `runner / target / symbol / selector / execution` の5列を必須とする。`execution` は `grouped` または理由を含む `isolated:<reason-slug>`。旧4列形式は受理しない。同一runner・設定・ファイルのテストをまとめ、Rustの通常featureとtest-deploymentは分離する。Rustはテスト一覧から完全修飾名を一意に解決してexact filterで直列実行し、JavaScriptとFoundryは構造化レポートを照合する。必須テストの欠落、skip、重複、失敗、結果の解析不能、異なるファイルの結果は拒否する。選択対象外のJavaScriptテストのskipだけは許可する。

`python3 scripts/check_claim_test_manifest.py --plan-only` はclaim、テスト、グループ、runner起動数、必要なビルドを実行前に表示する。`--validate-only` は登録検査だけを行う。`proofs-impacted` は生成したimpact JSONを `--impact-json` で渡し、対象claimの必須テストの和集合だけを実行する。Jestが選択されなければWasm依存を準備しない。全体 `proofs` は選択引数を渡さず全登録テストを実行し、部分実行から完全proof receiptは発行しない。
