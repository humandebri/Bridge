...
CIはこれらの全Lean theorem、Verus manifest、claim台帳、全vector section、全consumerを完全一致検査し、production linkはcompilerで型検査する。trusted PR proof gateでは、default branchのtrusted scriptが`current-main`または`security-hardening-v1`の完全なproof資材path＋SHA-256集合を一意に選択し、混在、欠落、余剰、未知digestを拒否する。集合にはVerus manifest、`pass.rs`、全negative fixture、SMTおよびHalmosのmanifestとpositive/negative harness、Halmos lock資材を含める。profileは候補側の環境変数では選択できず、profile間の検査fallbackも行わない。`current-main`は一時的な9-stage bootstrapであり、hardening merge後に旧schema経路とともに削除する。それまでは第三profileを追加しない。Solidity AST bindingはcommit state writeの演算子、mapping indexを含む左辺、右辺を完全一致させ、SMT production callを内包するassertionまたは後続のデータ・制御依存assertionを要求し、literal `assert(true)`を拒否する。
...
schema v35再オープンとwire v29をRust transaction testとsame-Wasm PocketIC testで検証する。Productionは旧schemaをfail closedにし、test-deploymentだけがreviewed relayerを伴うv34→v35移行を検証する。
