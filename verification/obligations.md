...
| confirmation jobと自動進行可能なscheduled jobは手動claimで迂回できず、stopped・overdue・expiredだけを明示回復できる | stable settlement executor manual claim | Lean定理、生成vector、Rust consumer、Verus共有predicate、storage test | SQLite atomic commit、async callback identity |
