...
def origin : AuthorizationOrigin := {
  finalizedBlock := 1, finalizedHash := 2, finalizedTimestamp := 0,
...
example : ExpiryEvidence.valid {
    ...
    verifyingContract := 3, depositProcessed := true, finalizedBlock := 2,
    finalizedHash := 5, finalizedTimestamp := 7201
