...
IC mainnet上の`sepolia-staging`で新規作成またはinstallするのはfresh targetの`bridge-sepolia-v35`だけとする。旧`bridge-sepolia` mappingは`abandoned-test-only` identityとして保持する。test tokenには既存の共有`testicrc` Canisterを再利用し、staging専用LedgerまたはIndex Canisterは新規作成しない。既存`testicrc`の実Canister IDとmetadataは外部stage開始前にlive状態から確認する。
