...
production開始には、Base contractの検証成功だけでは足りない。少なくとも、001〜006の初回activation範囲、初期運用値、対象SNS、認証済みGate A／Gate B、Sepoliaでの主要5 scenario、pause/cancel経路、鍵と監視の運用runbookが必要である。SNS Root単独controllerへのhandoverとSNS proposal upgradeは初回activationとは独立し、実施時期は運用者が別途承認する。Plan 007の追加wallet互換性と追加5 scenarioはnonblockingで継続する。
