...
通常の再オープンとProduction `post_upgrade`はstable schema v36、record wire version v29だけを受理する。test-deploymentだけは、live staging module hash、状態count、Governance principalを固定したupgrade argsで一方向のv33／wire v28→v36／wire v29移行を許可する。この移行はv33 DB内に残るwire v27の`ReserveGateChanged` audit eventも同名の履歴専用variantとして保持し、新規処理からは生成しない。その他の旧schema、未知schema、未知wire、decode不能なDBはfail closedで起動を拒否する。
upgrade検証はcurrent schema v36のrecord・config・quota・auditを保持するsame-Wasm再オープンと、旧schema・wireの拒否を検証する。
