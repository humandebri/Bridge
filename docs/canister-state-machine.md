...
通常の再オープンと`post_upgrade`はstable schema v34、record wire version v29だけを受理する。v33以下、未知schema、未知wire、decode不能なDBはfail closedで起動を拒否する。
upgrade検証はcurrent schema v34のrecord・config・quota・auditを保持するsame-Wasm再オープンと、旧schema・wireの拒否を検証する。
