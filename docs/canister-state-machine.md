...
通常の再オープンとProduction `post_upgrade`はstable schema v35、record wire version v29だけを受理する。test-deploymentだけは、承認済みconfirmation relayerをupgrade argsで受け取る一方向のv34→v35移行を許可する。その他の旧schema、未知schema、未知wire、decode不能なDBはfail closedで起動を拒否する。
