...
このrunbookは、旧stagingを`abandoned-test-only`としてactive profileから切り離し、schema v35／record wire v30の新規stackを構築する。旧stackの未処理test transactionは引き継がず、pause、upgrade、migration、reinstall、破棄を行わない。production Canister、KINIC Ledger、Base Mainnet、SNSを対象にしてはならない。
...
- active frontend profile、new signer、new Canister、new Bridgeのいずれからも旧identityを参照しないこと。

旧stackの値はstaging test stateとして放棄し、新stackへ移送、返金、再発行、再利用しない。未処理test transactionやunpaused状態は切替blockerにしない。旧Canisterと旧Base contractsには変更を加えず、UI、signer、profile、automationから到達不能にする。
...
- old stackがactive profile、signer、Canister、automationから完全に除外されている。
