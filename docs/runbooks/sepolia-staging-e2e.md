...
review済みlive v33の同一instanceからv36へのupgradeだけを`schema-migration-upgrade`として受理し、upgrade前後のstate count、
source schema v33／wire v28、target schema v36／wire v29、instance ID、`storage_integrity_check = ok`を照合する。異なるinstance、reinstall、v35、v34、v32以下、
...
v33→v36 upgradeではさらに、同じ観測時点の次のJSON artifactを保存する。
...
- install stageにはupgrade前後の全count、source schema v33／wire v28、target schema v36／wire v29、同一instance ID、`storage_integrity_check = ok`を記録し、いずれかが不一致ならUI公開へ進まない。
...
staging migration gateはlive stable schema v33／wire v28をv36／wire v29へ原子的に変換しながら、旧`get_public_config`を
...
BRIDGE_STAGING_IDENTITY=<identity> \
  scripts/plan007/staging-canister-upgrade.sh \
    ...
    --evidence /secure/work/staging-v33-v36-preflight.json
...
BRIDGE_STAGING_IDENTITY=<identity> \
  scripts/plan007/staging-canister-upgrade.sh --execute \
    --wasm "$(pwd)/target/test-deployment/staging/bridge_canister.wasm" \
    --local-evidence /secure/work/local-e2e.json \
    --preflight-evidence /secure/work/staging-v33-v36-preflight.json \
    --evidence /secure/work/staging-v33-v36-result.json
```
...
UIはCanister postcondition前にclean HEADから一度だけbuildし、repo外receiptへsource tree、profile、全asset hashを固定する。
Canister upgrade成功後は同じreceiptを再検証し、再buildせず`kinic-bridge-ui-test`へ公開する。

```sh
BRIDGE_STAGING_UI_RECEIPT=/secure/work/staging-ui-assets.json pnpm --dir ui artifact:test
BRIDGE_STAGING_UI_RECEIPT=/secure/work/staging-ui-assets.json pnpm --dir ui artifact:test:verify
# Cloudflare公開の明示承認後だけ実行する。
BRIDGE_STAGING_UI_RECEIPT=/secure/work/staging-ui-assets.json pnpm --dir ui deploy:test:artifact
```
