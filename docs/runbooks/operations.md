...
`verify-production-current-ui-live BUNDLE MODULE_SHA256 RPC_CONFIG UI_RUNTIME_PROFILE sole|joint` requires byte equality, live RuntimeBinding from signature-verified queries and module/controllers from certified `read_state`, Activated/unpaused state, no pending Base Governance, reserves, storage, and a fresh activation attestation.
