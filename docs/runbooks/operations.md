...
## 個人controllerを保持したDAO実証と最終移譲

本番公開後のDAO実証は[DAO再開手順](dao-reactivation.md)を使う。初回のseal／controller schedule／controller execute証跡は不変のまま保持し、DAOのschedule／execute receiptを別に保存する。再開後のhandoverとUI live検証には`BRIDGE_DAO_SCHEDULE_RECEIPT`と`BRIDGE_DAO_EXECUTE_RECEIPT`を両方指定する。片方だけの指定、proposal未実行、validator不一致、live activationとの不一致は拒否する。

個人controllerの削除は別途承認後だけに行う。Root-only変更後はSNSの管理対象登録を確認してから同一Wasm upgradeを行う。SNS管理対象登録を個人controller保持期間に実行してはならない。登録処理自体が個人controllerを削除する。

移譲後のUI公開では、既存checkpoint evidenceとRPC設定に加えて`BRIDGE_SNS_HANDOVER_RECEIPT`と`BRIDGE_SNS_UPGRADE_PROPOSAL_ID`を両方指定する。専用のSNS live検証はRoot-only controller、管理対象登録、upgrade mode、同一非圧縮Wasm、空Candid upgrade引数、proposal実行成功と、採択・移譲後のRootによるpost_upgrade完了記録を要求する。個人identityへRootを偽装させず、公開のread-only release queryから設定preimageとstorage integrityを取得し、従来と同じruntime・storage・history index条件を検証する。
