...
- Bridgeのcycles、Governance Operator／Runtime Administrator／Independent CancellerそれぞれのETH、governance reserve surplus、Finalized観測時刻、各roleのpending nonce lane、停止理由を確認する。さらに未決済Authorization、未決済Withdrawal件数・`amountOut`合計・最古観測時刻・Ledger停止理由を確認する。Mint Signer ETHは要件ではない。
...
新規install後、controller handover前にcontroller identityから`initialize_public_config()`を一度実行する。これはMint Signer、Governance Operator、Runtime Administrator、Independent Cancellerをchain-keyから導出し、すべて成功した場合だけstable stateへ一括保存する。続けて公開`get_runtime_binding()`、公開`get_control_plane_addresses()`、controller/governance限定`get_operational_config()`を実行し、release profileの4 role addressと一致することを確認する。`get_runtime_binding().operational_config_sha256`は、operational実値と固定Ledger feeをdomain-separated Candid encodingで束縛し、handover後の`verify-live`がprofile driftを公開値なしで検出する。初期化失敗、未初期化query、保存済みアドレスまたはbinding digestの不一致はいずれもdeployment blockerであり、空値や手入力値で代替しない。upgradeでは保存値を維持し、再初期化を必須としない。
...
- ETHはBase transactionを送信するGovernance Operator、Runtime Administrator、Independent Cancellerの各addressへ、そのroleが必要とする上限まで個別に補充する。Governance transactionは署名前に選択されたsender addressのSafe/Finalized残高の小さい方がそのtransactionの最大費用以上であることを検査する。Mint Signerへ補充せず、Deposit admissionやAuthorization発行にETHを要求しない。SNS-token feeの自動交換は行わない。
...
- Governance Operator、Runtime Administrator、Independent Cancellerのいずれかのnonce laneで競合した場合もCanisterは自動回復しない。CLIは該当roleの発行済みhashを確認し、exact raw transactionの再送または既知transaction応答だけを冪等成功として扱う。別hashによるnonce消費が疑われる場合は停止して監査し、どのroleのstable nonceも手作業で変更しない。

## Governance relayer

CanisterはBase governance transactionをbroadcastせず、governance timerも持たない。署名済みraw transactionの取得とbroadcastは匿名で行える。`confirm`と`run`はrelease profileへ固定した専用confirmation relayer identityを使い、障害時だけGovernance/Pause principalを復旧callerとして使う。初期配置はroleを残さない外部EOAで行う。Service Feeの署名要求にはGovernance principal identityを使う。初回activationはseal時に固定したproduction controllerのprepareまたは上限内replacement、匿名relay、固定confirmation relayer confirmへ分離する。初回execute確定時にbootstrap activation認可だけを永久に消費し、外部controller集合を変更しなくても以後はGovernance principalだけを認可する。
...
配置後のGovernance relayerは`status`を匿名で実行できる。初回activationはseal時に固定したproduction controllerによる`prepare-schedule-activation`／`prepare-execute-activation`、匿名`relay`、固定confirmation relayerによる`confirm`を別processで実行する。driverはlive Gate B合格直後かつCanister call前にauthorization receiptを耐久化するため、停止後はpendingを保ったまま同じprepareを冪等再開できる。新規prepareへ使うauthorizationは作成から5分以内を要求し、署名前の`Prepared`から再開するときも期限切れならfresh live Gate Bから新しいauthorization artifactを作る。relay/replacementは別途、5分以内のpaused activation attestationを要求する。既にBaseでFinalizedしたtransactionのconfirmだけは、このphase attestation freshnessを遮断条件にしない。confirm直前は固定artifact、authorization、prepare receipt、署名transaction、prior scheduleの結合を再検証し、さらにmanagement statusからproduction controller一件とcurrent profile Wasmを確認する。confirm成功後は固定confirmation relayerでactivation attestationを直ちに更新してから最終receiptを検証する。attestation更新または最終検証で停止してもraw confirmation receiptを保持し、同じconfirmを冪等再実行してBase transactionを再送せず復旧する。署名artifact生成後はGate B・authorization・artifact hashをprepare receiptへ結合し、relay/confirmはraw EIP-1559 transactionのhash・chain・sender・nonce・target・calldata・gas・feeを再構成してから検証済みprivate freezeだけを使用する。初回execute確定後のactivation `prepare`と`replace`はGovernance policyに従う。controller権限をService Fee、pause principal rotation、通常governance actionへ広げない。初回配置だけは暗号化Foundry keystoreと別password fileを入力とする`production-deploy-driver.sh`で行う。秘密、実path、RPC URLをrelease artifactやevidenceへ記録しない。
...
installと公開設定が成功しpostcondition収集だけが中断した場合は、元schema 2 plan、同一Wasm、残存する空の`/Volumes/KINGSTON/KINIC/bridge-production-prep-d85b7ce/production-canister-install-receipt.json.reservation`、sole installer controller、live module hashを確認してから、review済みの後続commitで同じcommandへ`--resume-post-install`を追加し、receiptには固定path`/Volumes/KINGSTON/KINIC/bridge-production-prep-d85b7ce/production-canister-install-receipt.json`を指定する。この明示分岐は今回の承認済み初回installだけに固定され、別receipt pathからの迂回を拒否し、元planのrevision/tree、canonical/file digest、init Candid digest、Wasm digest、Canister ID、installer principalの完全一致を必須にしてinstallを呼ばない。Gitのworktree/index/config/pathspec override、replacement object、graft、split/fsmonitor index、hidden index flagを拒否し、変更をinstaller・test・本runbookだけへ制限して、上限100 rowsのstorage validationと上限4,194,304 bytesのchecksum、全postconditionを再取得する。すべてのproject-awareなICP CLI callは、recipe解決と可変cacheをclean sourceから隔離するため、検証済みcommit archiveからprivate work directoryへ展開した専用project rootで実行する。schema 3 receiptは実際のinstallに使った元plan/sourceへ束縛したままにし、復旧driverのrevision/tree、元plan/Wasm、receipt writerへ渡したresponseのhash、receipt hashは`<receipt>.recovery.json`へ分離して保存する。復旧evidenceを先に、receiptを後に、検証済みinodeのno-overwrite publicationで確定し、両方とsourceを再検証できた場合だけ2個のreservationを削除する。evidenceだけが存在してreceiptまたはreservationが残った状態はterminal partialである。artifactやmarkerを削除して再試行せず、sourceと各hashを保全して手動調査する。復旧sourceは通常の独立した`.git` directoryを要求するため、linked worktreeではなく通常の隔離clean cloneから実行する。

固定receipt先のfilesystemはmacOSの`noowners`を許可しない。現在の`/Volumes/KINGSTON`を使う場合は、別途承認したうえでownershipを有効化し、mount rootを含む祖先からgroup/other writeを除去してからpreflightを再実行する。全祖先directoryはrootまたは実行者所有でなければならず、別UID所有のowner-writable directoryを経由してはならない。ownershipや祖先permissionを変更せずにsingle-user前提で検査を迂回してはならない。

復旧driverは現revisionが元install revisionの子孫であり、変更がdriver・test・本runbookだけであることをvalidator build前に検証する。その後review済みcommitをsanitized `git archive`へ一度だけ凍結し、そのsnapshotのmanifest、DID、mappingだけを使う。ignored file、Git common directory／alternate／attributes／local include、Cargo home config、compiler environment injectionを拒否または空環境で無効化し、validatorは固定Rust toolchain・offline `--frozen`でbuildする。inline Pythonはisolatedかつ`site`なしで実行する。PATH上の`icp`、Python、shasum、Cargo registry cache、Rust／Xcode／Homebrew toolchainは引き続きreview済みhost toolchainという外部前提である。snapshot作成からreservation cleanupまで、同じUIDの別writerと別hostのcontroller変更を禁止する。

artifact公開前とreservation cleanup直前にmanagement statusを再取得し、sole installer controllerとmodule hashが同じtupleであることを必須にする。公開前statusのraw digestとtupleはrecovery evidenceへ束縛し、公開後の再検証に成功した場合だけmarkerを削除する。`R`を元install reservation、`RR`をrecovery reservation、`E`をrecovery evidence、`P`をschema 3 receiptとすると、terminal stateは次のとおりである。

| 状態 | 意味 | 操作 |
| --- | --- | --- |
| `R` または `R + RR` | artifact公開前に停止 | 再実行せず手動調査 |
| `E + R + RR` | evidence公開後・receipt公開前に停止 | terminal partialとして全証跡を保全 |
| `P + R` または `E + P + R + RR` | receipt公開後・live cleanup検査前後に停止 | terminal partialとしてGate Aへ進めない |
| `E + P + R` | recovery marker削除後にcleanupが中断 | hash／inodeとlive stateを手動照合 |
| `P` または `E + P` | 全marker削除済み | installまたは復旧完了 |
...
post-activation production UI公開では、運用証跡とUI source証跡の役割を分離する。immutableなhistorical Gate B v35とseal／schedule／execute receiptは、配置時のCanister、runtime、activationを束縛する。`BRIDGE_POST_ACTIVATION_UPGRADE_EVIDENCE`はGate B終端moduleから現在のv36までのactivation後upgrade chainとする。既存v35 hotfix receiptを省略せず、今回のv35→v36 receiptを既存のhash-linked chain形式へ追加する。各receiptの署名済みinstall要求、Wasm、raw前後状態、時刻、module連鎖を検証し、Activatedとunpausedを維持する。v35→v35とv35→v36、その後のv36→v36だけを受理し、最終schemaは必ずv36とする。Gate B自身のprofileやmanifestをv36へ書き換えない。欠落、重複、順序逆転、別instance、未対応schema、runtime・会計・storage driftは拒否する。Gate B以前の既に検証済みprefixはpost-activation chainへ重複して含めない。
...
Canister更新と索引完成後に `bridge-profile render-production-ui-runtime BUNDLE SEAL SCHEDULE EXECUTE POST_ACTIVATION_CHAIN RPC_CONFIG OUTPUT` で公開用runtime profileを生成する。元Gate B manifestとprofile hashesに加え、upgrade chain bytesのSHA-256、検証済みv36 module/schema、RPC設定bytesのSHA-256を含む決定的描画を使う。`verify-production-ui-live BUNDLE SEAL SCHEDULE EXECUTE POST_ACTIVATION_CHAIN UI_RUNTIME_PROFILE RPC_CONFIG` はこのbyte一致と、chain終端に対するlive RuntimeBinding/module完全一致、installer単独controller、Activated、unpaused、pending Base governanceなし、reserve、storage、fresh activation attestationを検証する。さらに`list_withdrawals`の署名検証queryが成功するまで公開しない。`IndexNotReady`やRPC/API失敗を空の履歴として認可しない。

今回のclean sourceから生成するstandalone schema 2 UI asset receiptはWalletConnect project ID、全file digest、aggregate digestを束縛する。公開時に別途検証するruntime profileのupgrade/RPC digestが運用証拠を束縛する。controller専用storage integrity queryには`BRIDGE_PRODUCTION_INSTALLER_IDENTITY`を使い、解決principalがGate Bの単独controllerと一致しなければ拒否する。identity名や鍵素材はrelease evidenceへ記録しない。frozen assets作成後、Wrangler直前にlive認可とruntime profile bytesの不変性を再確認する。Gate B sourceとUI sourceの同一性や`BRIDGE_RELEASE_INPUTS_MANIFEST`は要求しない。asset・receipt・profile・upgrade chainのdriftではCloudflareへ送信しない。この経路はschedule／execute proposal、Canister upgrade、controller handoverを送信しない。
