...
stable schemaはv35、record wireはv30を現行形式とする。Productionとtest-deploymentの`post_upgrade`は現行形式だけを受理する。現在のstagingはCanister ID、deployment instance、minimum Withdrawal ID、Base contract bindingを保つcurrent-schema upgradeだけを受理し、一度限りのreinstall履歴を再開しない。
...
現行形式はstable schema v35／record wire v30とし、これ以外をfail closedで拒否する。stagingのactive bindingは既存Canister、deployment instance、minimum Withdrawal ID、Base contracts、signerへ固定し、target module・Candid hash、controller、state count、storage integrityをupgrade前後で照合する。初期化済みの永続Canisterは同一deployment instanceのcurrent-schema upgradeだけで更新し、reinstallは禁止する。
...
現在のstagingは、一度限りのreinstall/fresh-stack履歴をimmutable evidenceへ固定し、同じCanister、deployment instance、Timelock、Bridge、bSNS、signerを維持する。今後のreinstall、契約再deploy、instance変更は行わない。RPC rehearsalで一時pauseした場合は、別operationのreactivation receiptと全pending countゼロを確認した後にだけ`SHORT_DELAY_LIVE`へ戻し、資産フローを有効に保つ。
