...
post-activation production UI公開では、運用証跡とUI source証跡の役割を分離する。immutableなhistorical Gate Bとseal／schedule／execute receiptは配置済みCanister、runtime、activationを束縛し、今回のclean sourceから生成するstandalone schema 2 UI asset receiptはWalletConnect project ID、全file digest、aggregate digestを束縛する。`bridge-profile verify-production-ui-live`はhistorical Gate Bから決定的にrenderした`ui-runtime-profile.json`とのbyte一致に加え、Activated、v35、installer単独controller、module、unpaused、pendingなし、reserve、storage、live activation confirmation、fresh attestationを検証する。通常deployはfrozen assets作成後かつWrangler直前にこのlive認可を再実行する。Gate B sourceとUI sourceの同一性や`BRIDGE_RELEASE_INPUTS_MANIFEST`は要求しない。asset追加・欠落・hash drift、receipt/profile driftではCloudflareへ送信しない。この経路は完了済みschedule／execute proposalを再送せず、Canister upgradeやcontroller handoverも行わない。
