...
配置後のGovernance relayerは`status`を匿名で実行できる。初回activationはseal時に固定したproduction controllerによる`prepare-schedule-activation`／`prepare-execute-activation`、匿名`relay`、固定confirmation relayerによる`confirm`を別processで実行する。driverはlive Gate B合格直後かつCanister call前にauthorization receiptを耐久化するため、停止後はpendingを保ったまま同じprepareを冪等再開できる。ただしauthorizationの有効期間は5分であり、署名前の`Prepared`から再開するときも期限切れならfresh live Gate Bから新しいauthorization artifactを作る。このfreshnessはprepareとrelay/replacementの送信前条件であり、既にBaseでFinalizedしたtransactionのconfirmを遮断する条件にはしない。confirm直前は固定artifact、authorization、prepare receipt、署名transaction、prior scheduleの結合を再検証し、さらにmanagement statusからproduction controller一件とcurrent profile Wasmを確認する。confirm成功後は固定confirmation relayerでactivation attestationを直ちに更新してから最終receiptを検証する。attestation更新または最終検証で停止してもraw confirmation receiptを保持し、同じconfirmを冪等再実行してBase transactionを再送せず復旧する。署名artifact生成後はGate B・authorization・artifact hashをprepare receiptへ結合し、relay/confirmはraw EIP-1559 transactionのhash・chain・sender・nonce・target・calldata・gas・feeを再構成してから検証済みprivate freezeだけを使用する。初回execute確定後のactivation `prepare`と`replace`はGovernance policyに従う。controller権限をService Fee、pause principal rotation、通常governance actionへ広げない。初回配置だけは暗号化Foundry keystoreと別password fileを入力とする`production-deploy-driver.sh`で行う。秘密、実path、RPC URLをrelease artifactやevidenceへ記録しない。
