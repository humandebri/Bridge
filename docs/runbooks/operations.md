...
現行の開発・staging・production canisterはstable schema v32／record wire v28だけを受理する。これ以外の形式は空stateであってもfail closedとし、migrationや旧Wasm fixtureを現行release判断へ使用しない。初期化済みの永続Canisterは同一deployment instanceのupgradeだけで更新し、reinstallは禁止する。新しいCanister IDへの初回installはこの制約に含めない。
