...
- If handover occurs at a separately approved time, require clean current source, complete proofs, a twice-reproduced current Wasm, and authenticated live RuntimeBinding/state. Immediately before submission, require the production identity as sole controller, Activated state, and unpaused flows. Preserve module, runtime, storage integrity, and operational continuity; afterward require exactly the production identity and SNS Root as joint controllers. Dapp registration and production-controller removal remain separate operations.
