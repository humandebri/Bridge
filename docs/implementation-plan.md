...
Bridge canisterはstable schema v23、外部連携、Settlement Reserve、stable settlement executor、Deposit用wallet確認付きフロント通知、運用管理、Verus証明まで実装済みである。
...
後続のPlan 002と003およびADR 0017から0022で外部連携、運用状態、settlement executor、フロント通知型confirmation、fund-before-formal-depositを追加し、現行stable schemaはv23である。
...
- schema versionは`bridge_metadata`だけを正本とし、現行形式はschema v23・record wire v19とする。
