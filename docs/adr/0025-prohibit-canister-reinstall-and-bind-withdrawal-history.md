...
初期化済みの永続Canisterは、同じdeployment instanceを保つreview済みstable schema v35／record wire v30から同じ現行形式へのupgradeだけで更新する。reinstall、instance変更、旧schema、未知schema、未登録wireはdeployment gateとstorage reopenで拒否する。旧stagingは`abandoned-test-only`としてactive profileから除外し、新しいCanister IDへのv35／wire v30初回installで置換する。
...
- production未配置の旧staging schemaや履歴を失ったstateはmigrationせず、旧stackを`abandoned-test-only`としてactive profileから除外して現行schemaの新規Canisterへ検証stateを作り直す。
