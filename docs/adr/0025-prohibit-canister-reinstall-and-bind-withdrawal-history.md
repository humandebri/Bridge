...
初期化済みの永続Canisterは、同じdeployment instanceを保つreview済みstable schema v35／record wire v30から同じ現行形式へのupgradeだけで更新する。legacy migration例外は設けない。reinstall、instance変更、旧schema、未知schema、未登録wireはdeployment gateとstorage reopenで拒否する。新しいCanister IDへの初回installだけは許可する。
...
- 旧schemaや履歴を失ったstateは復旧対象にせず、現行schemaで検証stateを作り直す。
