...
Gate AはBootstrap運用値でのCanister導入とpause状態のBase配置完了を表す。初回activation前はlive情報から導出した初期運用値を一回限りsealし、主要RPC rehearsal、monitor drill、pause/cancel経路とpre-seal／live Gate Bを完了する。7日以上のBase feeと各10件以上の本番governance gas／settlement cycles、keeper drill、monitoring receipt、handover証跡はunpause後のGate Cへ分離する。計測結果は運用値を自動更新せず、controller handoverの時期も自動決定しない。
