...
TimelockをBridgeより先にdeployし、初期minimum delayを72時間、Canister由来Governance Operatorをproposer、executor、canceller、追加adminを`address(0)`として構成する。
...
- 外部EOAからBridgeのBase Admin関数を直接呼んでも失敗し、CanisterがTimelockへscheduleして72時間後にexecuteする。
- Bridgeは候補addressのbytecode、`getMinDelay() >= 72 hours`、候補自身の`DEFAULT_ADMIN_ROLE`保持をrotation時に検証する。
