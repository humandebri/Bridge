...
| Bridge Canister | SQLite schema v29、Ledger操作、EIP-712署名、Finalized照合、Governance transaction署名 |
...
7. Base transaction送信後、UIはreceiptと`DepositMinted` eventを`Submitted`、`Confirmed`、`Finalized`まで追跡する。成功時のIC wallet署名は要求しない。reload後もCanisterのDepositとFinalized Base logをDeposit IDで統合し、exact digest、recipient、gross amount、service fee、mint amountが一致する成功だけを復元する。
